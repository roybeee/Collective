...
마지막 갱신: 2026-09-25 06:05 UTC (Claude 품질 계획 v2 세션, G0·운영 회의 캡처 8건·합성 dev S6~S8)

## 현재 운영 상태

| 항목 | 값 | 근거 |
|---|---|---|
| 운영 제품 커밋 | `ac7a0310c2d20614a1c364b86ee53cc3eb04764c` (#90 `merged`, 묶음 7: 443fff4 뒤 #81~#90, 묶음 6 `a41624d` 포함) | 제품 tree `53492af7dea93d68388b4addcb33558dcf125cac` |
| `origin/main` | `d08e7eb` (#93) 이후 문서 PR. ac7a031 뒤 변경은 비제품 경로(문서·테스트·`scripts/eval/`)뿐 | `gh`, 2026-09-25 |
| Sites 게시 | `published` 버전 29, deployment `appgdep_6ab5e9ec3ac4819192483b1e70993816`, Sites 커밋 `01b6e05`, 파일 목록·기대 해시 방식(커넥터, v28→ac7a031 61줄 목록) | [게시 기록](releases/2026-09-25-ac7a031.md) |
| 실행 검증 | `runtime-verified` · real, 2026-09-25 03:30 UTC 경 | 소유자 이메일 세션 `/api/version` tree `53492af…` = ac7a031 tree, build 2026-09-25T03:25:38Z, `promptManifest` null. 익명 401, `/media` 404·CSP 확인 |
...
- 골든셋 v2 dev 확대(2026-09-25): G0(real, 토큰 0) 결과 운영에는 ODA 역할 3·실패 회의 1(완료 단계 4), MAPDAL 역할 8·완료 회의 1(13단계)이 있고 브리프 초안은 0건이다. 운영 회의 단계 8건을 dev로 캡처했다(real, 평가 케이스 쓰기: MAPDAL 6·ODA 2, 모두 `code_changed`, ODA 2건 `frozenIdentical:false`). C1이 3역할뿐이라 설계대로 합성 dev S6·S7에 S8을 더했다(PR `feat/golden-dev-s6-s8`: S6 무인 보관함 locker, S7 분식 자판기 가맹 B2B fnb, S8 필라테스 체험 education, 각 역할 8). 병합 뒤 운영 트리(64e51e8) 체크아웃에서 생성해 가져오면 dev는 역할 51·회의 단계 20·브리프 3 = 74케이스다(설계 72, 브리프 3건 부족을 역할·회의가 메운다). 봉인 S1·S4·S5는 프롬프트를 고치는 레인이 아닌 사람이 만든다(대기).
