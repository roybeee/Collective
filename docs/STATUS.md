...
마지막 갱신: 2026-09-25 22:34 UTC (Codex: A1 후보, GitHub 403 복구·브랜치 생성, 전체 검사 passed, 운영 로그인 미완료)

## 현재 운영 상태

| 항목 | 값 | 근거 |
|---|---|---|
| 게시된 제품 커밋 | `8c22f0e8738653698dd4350cd2063e3755340c83` (#115) | source tree `02260ba4d4bf50dbd617fd4f6dc2709c5e067cf2`; 실행 중 tree는 이번 확인에서 blocked |
| `origin/main` | `41ea80d008212b5b28796d4d9e0307013592ccbc` (#117) | git fetch 및 ls-remote / GitHub 커넥터 조회 일치, 2026-09-25 21:30 UTC. #117 제품 변경은 버전 35에 미포함 |
| Sites 게시 | `published` · real, 버전 35, deployment `appgdep_6ab68fba2a9881918648291e5ba1e40d` succeeded, 환경 revision4 | [게시 기록](releases/2026-09-25-8c22f0e.md), Sites API 사후 확인 |
| 실행 검증 | `blocked` · real, 2026-09-25 22:34 UTC | 운영 루트 로그인 화면은 접근 가능. 인증 전 `/api/version` 요청은 Worker 로그에 401로 남았다. 안전 로그인 제출은 실패해 실행 tree를 확인하지 못함 |
...
| 진행 PR | #16 Android(draft, 제외). A1은 D1 대조 전 draft 대상 | GitHub 403 원인인 앱 허용 저장소 누락을 사용자가 수정. `feat/a1-local-channel-pack` 브랜치 생성 성공 |
| main CI | 최신 #117에 대한 이번 세션 조회는 `not_run`. 과거 `8ed30d0`·`8c22f0e`·`0443c9f` passed 기록은 유지 | 과거 GitHub Actions 조회 2026-09-25 15:52 UTC; 이 PR의 로컬 검사는 아래 A1 항목 |
...
- Codex `feat/a1-local-channel-pack` (기준 `41ea80d008212b5b28796d4d9e0307013592ccbc`, 2026-09-25 21:30 UTC): A1 로컬 채널 콘텐츠 후보와 출처·수용 기준, 직전 게시 기록을 정리한다. 소유 파일은 `prompts/channel.offline.json`, `tests/check-prompts.test.mjs`, `tests/prompt-resolution.test.mjs`, `tests/local-channel-practice.test.mjs`, `docs/LOCAL-CHANNEL-PACK.ko.md`, `docs/STATUS.md`, `docs/HANDOFF.ko.md`, `docs/releases/2026-09-25-8c22f0e.md`다. 운영 등록·쌍 평가·활성화는 아직 실행하지 않았다. 정적 검사·빌드 및 관련 회귀 passed. 코드 동일성 가정 1건을 후보 절차로 수정하고 전체 러너 128/128 스위트, 8,986 assertions passed. GitHub 쓰기 403은 복구돼 원격 브랜치를 생성했다. 운영 로그인 미완료로 D1 대조·재채점·활성화는 blocked/not_run이며 D1 대조 전 draft 상태를 유지한다. [검증·후속](LOCAL-CHANNEL-PACK.ko.md).
