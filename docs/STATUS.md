...
마지막 갱신: 2026-09-23 01:12 UTC (Claude, 브랜치 `docs/status-hermes-tools`)
...
1. **실제 실행 관측(PR #6 `merged`)의 남은 미확인.** 후속 관측(2026-09-23 00:55 UTC, real, 읽기 전용)으로 2건을 해소했다(`docs/observations/2026-09-23-live-run.md` 5절).
   - `viral_discovery` 실패 원인: 당시(9/17) HERMES api_server에 도구가 없어서 모델이 사례를 지어내지 않고 비워 반환했고, 앱이 이를 `failed`로 기록했다. 도구는 9/18에 연결됐다(다음 행동 1).
...
| PR | 브랜치 | 내용 | 상태 |
|---|---|---|---|
| — | `docs/status-hermes-tools` | HERMES 검색 도구 연결 상태 확인 결과 반영 | PR 진행 중 |

## 다음 행동

1. HERMES 검색 도구는 이미 연결돼 있다(2026-09-23 01:10 UTC 확인, real). 서버 `~/.hermes/config.yaml`의 `platform_toolsets.api_server`는 9/17 실행 당시 `[no_mcp]`였고, 2026-09-18에 `[browser, web, no_mcp]`로 바뀌었다(web backend firecrawl, browser backend `off`). 서버 안에서 `/v1/runs`로 점검 실행 1회(15,177 토큰)를 돌려 `web_search`·`web_extract`가 노출되고 실제 URL을 돌려주는 것을 확인했다. Collective 코드는 바꿀 필요가 없다(프롬프트가 공급자와 무관하다). 남은 일: 앱에서 `viral_discovery`를 1회 다시 실행해 끝까지 검증할지 소유자가 결정한다(유료, 운영 쓰기). 게이트가 배포된 상태에서 quality를 실행하면 `reportedVerdict` 관측도 가능하다.
