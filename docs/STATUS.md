...
마지막 갱신: 2026-09-24 19:33 UTC (Claude 트랙 R 계획 세션, OFD 소개서 수정 지시서)

## 현재 운영 상태

| 항목 | 값 | 근거 |
|---|---|---|
| 운영 제품 커밋 | `df7e25386e06826d0ff40ac96a488d19a8539450` (#76 `merged`, 묶음 4: aa999c6 뒤 18커밋 #59~#76) | 제품 tree `0d074b53bbd39c32c233d48ce39be2e4a7e3e18f` |
| `origin/main` | `e2f6142` (#79) | df7e253 뒤 **미게시 제품 변경 있음**: #78 조사 근거 재선언 보존, #79 조사 의뢰 정보 가림 전송. main 기준 `runtime-verified` 아님 |
| Sites 게시 | `published` 버전 27, deployment `appgdep_6ab5370f6cd08191adcbbd91afe397c7`, Sites 커밋 `1602357`, 2026-09-24 14:41 UTC 경. Sites 작업 환경의 GitHub fetch가 막혀 커넥터로 파일 126개를 받아 tree를 맞췄다 | [게시 기록](releases/2026-09-24-df7e253.md) |
| 실행 검증 | `runtime-verified` · real, 2026-09-24 15:08 UTC 경 | 소유자 이메일 세션 `/api/version` tree `0d074b5…` = df7e253 tree, `promptManifest` null. 익명 401, `/media` 404·CSP 확인 |
| 인증 | `AUTH_MODE=email`, 계정 1개(소유자) | 운영 `/api/auth` mode=email, role=owner (2026-09-24 15:08 UTC 경) |
| Sites 접근 | public(사용자 명시 승인, 접근 설정 revision2). 이번 게시도 기존 접근 설정 유지 | 게시 에이전트 보고 "기존 공개 접근 설정을 유지". 게시 뒤 접근 설정 재확인은 not_run |
| 조사 워커 | online(lastSeen 2026-09-24 15:07 UTC, blocked 0, rotationReady true). 2026-09-24 14:15 UTC 새 설치기로 재설치(격리 점검 전부 통과) | `/api/research-worker/setup` 조회(real). gate 표시 `missing`이라 `RESEARCH_WORKER_APP_GATE=enforce`는 켜지 않는다 |
| 열린 PR | #16 Android(draft, 제외) | GitHub 열린 PR 목록(GitHub MCP), 2026-09-24 17:06 UTC |
| main CI(`df7e253`) | passed · verify·e2e-smoke | GitHub Actions main 실행 |
...
- 확인 필요: 익명 업무 API 401은 df7e253 게시 뒤 확인했다(passed · real). 위조 헤더 요청 401은 not_run이다(자동 모드 안전 검사 정책). 소유자가 직접 확인한다. 민감 작업 재인증(step-up)은 아직 구현되지 않았다(PR #23 남은 위험).
...
- 품질 우선 결정(2026-09-24 15:1x UTC, 대표 선택지 답 — '우리가 산출해낸 결과는 최고의 품질이어야만 한다'): 품질 기준선 측정(B5 서버 평가), 온라인 채점 켜기, 심층 조사 수리 턴 켜기, 조사에 의뢰 목적·시장·경쟁사를 가림 뒤 다시 보내기 — 네 가지 모두 승인. 운영 기능 스위치 `online_grading`·`a7_repair_turn`을 2026-09-24 15:13 UTC 경 켰다(소유자 세션 `POST /api/feature-flags`, 응답 200, 조회 결과 override=true). 평가 연결은 아직 없다(`/api/eval` connection configured=false) — 대표가 평가 키로 등록한 뒤 케이스 캡처·실행. 조사에 의뢰 목적·시장·경쟁사 다시 보내기는 PR `feat/research-intake-masked`(기준 `7b45491`, `merged` 전)에서 진행 중이다: 조사(R1·R2) 입력 브랜드 `request`에 세 필드를 `lib/pii-scan.ts`로 가려 싣는다(탐지 0이면 원문 그대로이고 따로 자르지 않는다, 소개·메모와 학습·제작 경로는 계속 제외, `docs/DATA-PROCESSING.ko.md` 4.4 ⑤). 운영 반영은 병합·게시 뒤다.
