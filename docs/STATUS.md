...
마지막 갱신: 2026-09-25 09:24 UTC (Claude 품질 계획 v2 세션: R3 기준선 완료, 측정 오탐 수정 PR)

## 현재 운영 상태

| 항목 | 값 | 근거 |
|---|---|---|
| 운영 제품 커밋 | `64e51e845d09dda821b4a09cab405f4374e9b616` (#98 `merged`, 묶음 8: ac7a031 뒤 #91~#98, 제품 변경은 `lib/graders/`뿐) | 제품 tree `449a67740a878885df1732539ef022b38d5e3c1c` |
| `origin/main` | `4eb16a3` (#99) | 64e51e8 뒤 #99. 제품 경로 변경 여부는 다음 게시 때 `docs/PUBLISH.ko.md` 5단계 명령으로 판정 |
| Sites 게시 | `published` 버전 30, deployment `appgdep_6ab6007ce11481918f6ae9a5aa250c49`, Sites 커밋 `ed7aa13`, 파일 목록·기대 해시 방식(커넥터, ac7a031→64e51e8 21줄 목록) | [게시 기록](releases/2026-09-25-64e51e8.md) |
| 실행 검증 | `runtime-verified` · real, 2026-09-25 05:39 UTC 경 | 소유자 이메일 세션 `/api/version` tree `449a677…` = 64e51e8 tree, build 2026-09-25T05:01:24Z, `promptManifest` null. 익명 401, `/media` 404·CSP 확인 |
...
| 열린 PR | #16 Android(draft, 제외), 트랙 R PR(브랜치 `claude/franchise-recruitment-marketing-u8cpo2`, 이 갱신과 같은 PR) | GitHub 열린 PR 목록(GitHub MCP), 2026-09-25 06:31 UTC |
...
- 로컬 채널 결정(품질 수정 v2, PR `fix/local-channel-decisions`): R3에서 점포 목표 합성 캠페인(S2·S6·S8)의 총괄·전략 6건이 모두 `local_channel_coverage` 1/4였다. 현장 스킬(`channel.offline`, 코드 상수와 `prompts/` 정본)에 로컬 채널 4개군(네이버 플레이스·당근·배달앱·카카오)을 한 줄씩 채택·후순위·제외로 결정하게 했다. 역할 제출 바이트가 점포 캠페인에서 바뀌어 기준 fixture를 `73a90ed`에서 재캡처했다. 게시 뒤 6케이스를 다시 돌려 확인한다(약 0.1M).
...
- 트랙 R(가맹점 모집 마케팅 솔루션, [계획](FRANCHISE-RECRUITMENT-PLAN.ko.md)): 대표 요청(2026-09-24)으로 계획을 세우고 1차 대상을 OFD 첫 가맹으로 정했다(결정 19). OFD 운영 사실·계약 내용·모집 자료 개정본·법률 미팅 자료·계약서 검토서는 저장소 밖에 둔다(공개 저장소 경계). 저장소에는 건수만 적는다: 모집 자료 개정 초안 2건과 브랜드 소개판 1건, 법률 미팅 자료 5건, 가맹계약서 검토 1건(필수 기재 13개 호 중 충족 9·미흡 3·누락 1, 위험 30건, 2차 검토 유지 71·기각 1). 정보공개서 등록 최신판은 공정위 사이트로 확인했다(passed · real, 2026-09-25 03:44 UTC). R0 판정은 '판정 불가'다(남은 확인 항목 있음). 대표 결정(2026-09-25): 결정 20 법률 검토 보류(LR-1·LR-2 `blocked`, 게이트는 휴리스틱 표시), 결정 22 리드 이름·연락처 저장(R8 연락처 원장을 R4b에 합침, 공개 수집 R10은 조건부 유지). 구현(이 브랜치, `main` 병합 전): R4a 순수 규칙·게이트(규칙 54, 사유 코드 24·경고 3), R1a 가맹 프로필·정보공개서 버전·계약서 템플릿·개인정보 고지 레지스트리, R4b 리드 원장(`/api/franchise`, 연락처 필드 암호화 `v1.`·HKDF-HMAC 중복 키·담당자 범위 열람·열람과 내보내기 감사·추가 전용 증빙·게이트 우회 없음·180일 파기·정보주체 요청, 화면 '가맹 모집'). 스위치 `r_franchise` 기본 꺼짐, D1 migration·새 환경변수 없음. 검사(main `4eb16a3` 병합 트리): 아래 PR 본문과 같다. 실제 브라우저 확인·법률 적합성은 `not_run`. 대표가 1~7단계(커밋·PR·병합·게시)를 승인했다(2026-09-25). 게시 실행은 Sites 도구가 있는 편집기에서 대표가 한다.
