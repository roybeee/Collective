...
마지막 갱신: 2026-09-25 01:52 UTC (Claude 품질 계획 v2 세션, AI 심사 실행 J3)
...
- AI 심사 실행(품질 계획 v2 J3, PR `feat/judge-runs`, J2 위): `start_run variant:'judge'`가 보정 라벨이 있는 평가 출력만 J1 루브릭으로 심사한다(예약 25,000, 월 예산 합산, 금지 값이 든 항목은 보내지 않음, 결과는 점수만·인용과 가린 이유는 `judge_output`). `?judge=<run>`이 기준별 보정 통계·채택 판정을 낸다. 검증 mocked(29). **대표 결정 대기**: 데이터 처리 E1 확장(G2 회의·브리프 동결, J3 심사 재전송) — 승인 전에는 회의·브리프 캡처와 심사 run을 시작하지 않는다.
