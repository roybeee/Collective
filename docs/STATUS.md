...
마지막 갱신: 2026-09-25 02:33 UTC (Claude 트랙 R 계획 세션, R4a 구현. main 병합)
...
| `origin/main` | `4a39dec` (#91) | 443fff4 뒤 **미게시 제품 변경 있음**: #82~#90(묶음 6 `a41624d`, 묶음 7 `ac7a031` 게시 승인 대상). main 기준 `runtime-verified` 아님 |
...
| 열린 PR | #92 교육 캠페인 합성 개발 명세(S3), #16 Android(draft, 제외) | GitHub 열린 PR 목록(GitHub MCP), 2026-09-25 02:33 UTC |
...
- 대표 승인(2026-09-25 02시 UTC 경, 선택지 답): ① 묶음 7 `ac7a031` 게시 승인(J1·G3·Q1·Q2·G2·G4·J2·J3, 목표 tree `53492af`, D1 migration·의존성·환경변수 변경 없음) — 커넥터 방식 지시문을 대표에게 보냈다. Sites가 버전 29(`a41624d`)면 46줄, 버전 28(`443fff4`)이면 61줄 목록을 적용한다(묶음 6 게시 여부를 확인하지 못해 두 경우 모두 적었다, 두 목록 모두 목표 tree 재현 검증). ② E1 확장 승인(위). ③ 운영 HERMES 기본 프로필 무응답 한도 90초 → 300초 승인 — 대표가 운영 작업이 없을 때 서버에서 실행한다(명령 전달, not_run). 게시 결과·운영 한도 변경 결과를 받으면 runtime-verified 확인과 릴리스 기록을 남긴다.
...
- 트랙 R(가맹점 모집 마케팅 솔루션) 개발 계획: 대표 요청(2026-09-24)으로 `docs/FRANCHISE-RECRUITMENT-PLAN.ko.md`를 브랜치 `claude/franchise-recruitment-marketing-u8cpo2`에 작성했다(기준 `df7e253`, 병합 전, 문서만). 대표 지시(2026-09-24, "계획은 좋아, 그런데 첫번째 목표는 mapdal의 첫 가맹이 아니라 올드페리도넛(OFD)의 첫가맹으로 진행하라")로 결정 19의 트랙 R 채택과 1차 대상 OFD가 정해졌다. 나머지 결정 19 항목과 결정 20~31, 에픽 R0~R15는 제안이다. OFD R0 준비도(2026-09-24): 대표 답변 6항목과 대표가 올린 자료 4건(등록 정보공개서 공개본 1건, 모집 자료 3건)을 확인했고 값은 저장소 밖(D1 등록 예정)에 둔다. 5번 중 가맹점 수·가맹금 예치/피해보상보험은 등록 정보공개서 공개본으로 확인했고, 중소기업 여부는 공개본 수치로 추정만 했다(중소기업확인서 미확인). 가맹계약서 13개 호 대조는 계약서 원본을 받지 못해 `blocked`다. 분기 판정(A/B)은 등록 버전 유효 확인과 계약서 대조 뒤 결정 21로 기록한다. 모집 자료에서 모집 전에 고칠 항목과 법률 검토 1차 패킷에 넣을 절차 위험 항목을 대표에게 따로 보고했다. 계획 문서에는 일반 규칙 3곳(본계약 전 점포 개발 약정, 수익 시나리오의 3.3㎡당 비교, Q7)만 더했다. 대표 지시(2026-09-24, "개발제안서는 법적으로 맞게 세팅하자")로 모집 자료 1건(개발 제안서)의 개정 초안, 동반 양식 1건, 적용 메모를 저장소 밖에 만들었다(법률 검토 전, 발행 전 확인 항목 33건, 대표 결정 8건). 발행은 확인 항목 해소와 LR-1 회신(또는 결정 23 위험 수용) 뒤다. 이어서 모집 자료 1건(가맹사업소개서 18쪽)의 쪽별 수정 지시서와 시안 5쪽을 저장소 밖에 만들었다(감사 지적 94건, 발행 전 확인 28건). 두 판(공개용·상담용) 모두 분기 A 판정 전에는 배포하지 않는다. 계획의 금지 표현 등급표에 협업 표기·IP 귀속·연혁 연수 행을 더했다. 대표 지시(2026-09-24, "1단계 브랜드 소개판을 뽑아라")로 가맹 조건이 없는 브랜드 소개판 1건(8쪽)과 사용 안내를 저장소 밖에 만들었다. 분기 A 전에는 브랜드 자체 채널과 요청 1회 발송에만 쓰고, 사용 전 확인 5건이 남았다. LR-0 준비: 대표 지시(2026-09-24, "만들어줘")로 2026-09-25 변호사 미팅용 자료 5건(1쪽 요약, 현장 카드, 전체 질문지, 기록지, 대표 준비 목록)을 저장소 밖에 만들었다. 법률 검토 회신은 `not_run`(미팅 전)이다. 코드 변경·게시 없음. 첫 행동은 코드 0의 R0 준비도 진단·수동 절차서와 순수 모듈 R4a이고, 트랙 R 제품 코드는 묶음 4~7에 들어가지 않았고(묶음 6 `a41624d`, 묶음 7 `ac7a031` 승인) 가장 이르면 묶음 8에 실린다. R4a(순수 규칙·게이트 모듈)를 이 브랜치에 구현했다(2026-09-25, 대표 지시 "원래 목표를 위해 진행하라"): `lib/franchise-rules.ts`(규칙 54개), `lib/franchise-gates.ts`(사유 코드 24·경고 3), 테스트 2개 스위트. `main` 병합 전이고 런타임 연결이 없다. 검사: `node scripts/test.mjs` passed · mocked(109/109, 6992 assertions, 새 스위트 136·311), tsc passed, lint-gate 기준선과 같음(오류 70·경고 39), build passed, check-prompts passed. 법률 적합성은 `not_run`(LR-1). 계획 H2에 말일 주말·공휴일 연장을 더해 보수적으로 강화했다.
