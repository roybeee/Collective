...
마지막 갱신: 2026-09-24 KST (F4b 초안, 검증 지적 반영. 근거는 이 작업 트리의 코드다)
...
| 기능 | 받는 곳 | 국외 여부(근거/확인 필요) | 보내는 항목 | 개인정보 가능성 | 언제 | 현재 보호 조치 | 근거(file:line) |
|---|---|---|---|---|---|---|---|
| **H1** 앱 호스팅과 저장 | ChatGPT Sites 게시, Cloudflare D1(`DB`)·R2(`BUCKET`) 바인딩 | 확인 필요. 리전과 운영 주체(OpenAI의 Sites 수탁 범위, Cloudflare 재수탁 여부)가 기록돼 있지 않다 | 앱의 모든 기록: 브랜드, 자료 원본(8MB 이하 파일, 이미지 포함)과 추출문(80,000자), 주문 장부, 작업물과 검토 메모, 전송 원문(`hermes_submission`·`openai_submission`), 평가 케이스·출력 | 높음. 업로드 내용에 제한이 없고 개인정보 검사도 없다 | 앱을 쓰는 동안 계속 | 텍스트 추출은 브라우저에서 하며 외부 추출 서비스를 쓰지 않는다. 연결 키를 암호화해 저장한다. 다운로드는 첨부 형식으로, sandbox와 no-store를 적용해 내려준다 | `docs/PUBLISH.ko.md:3`; `.openai/hosting.json:1-5`; `cloudflare-env.d.ts:3-4`; `app/api/archive/file/route.ts:6-15`; `lib/archive-upload.ts:1-6`; `lib/server.ts:42-44` |
...
`orderRefs`의 주문 레코드 id는 `[지점 id, 주문 출처, 주문일, 주문번호]`를 소금 없이 SHA-256한 값이다(`lib/store-operations-server.ts:23`). 입력 공간이 작으면 주문번호를 거꾸로 맞춰 볼 수 있으므로 가명정보에 해당하는지를 법률 검토에 올린다(6절 5번). 모델 과업에 이 값이 필요하다는 근거는 찾지 못했다. 그래서 제외 후보로 둔다(4.4 ②).

### 3.3 브랜드 자료·업로드

| 데이터 | 외부로 가는가 | 어디로 | 근거 |
|---|---|---|---|
| 업로드 원본 파일 | 모델로는 가지 않는다. 호스팅(R2)에 저장한다 | H1. R2 원본을 읽는 곳은 다운로드 GET뿐이다 | `app/api/archive/file/route.ts:6,13` |
...
| 텍스트 추출 과정 | 외부로 가지 않는다(브라우저에서 같은 출처의 PDF 워커로 추출) | 없음 | `lib/archive-upload.ts:1-6` |
...
- **브랜드 레코드에 허용 목록이 없다.** 조사·학습은 브랜드 레코드 전체를 보낸다(`lib/research-execution.ts:47,63`, `lib/learning-execution.ts:49`). 제작의 `aiBrand`도 구조만 바꿀 뿐 `id`·`bg`만 빼고 소개·메모(최대 30,000자)·의뢰 정보·`audience`·`constraints` 자유 텍스트를 그대로 보낸다(`lib/ai-context.ts:16-19`, `lib/agency.ts:2`). 그래서 경로 사이의 최소화 차이는 사실상 없다.
...
- **모델 과업에 필요한 근거가 없는 값이 나간다.** `orderRefs`가 그렇다(3.2).
...
- **입력 화면에 안내가 없다.** 검토 메모 입력란(`app/api/action/route.ts:40`)과 자료 업로드 화면에 개인정보 입력 금지 안내가 없다. `app`에서 '개인정보'를 검색한 결과는 0건이다.
...
- **파기 경로를 찾지 못했다.** 대상은 조사·학습 제출 원문과 학습 응답(`lib/record-kinds.ts:51,54`), 자료(`lib/record-kinds.ts:28`), R2 원본이다. `runtime.BUCKET.delete`는 업로드 실패를 되돌릴 때만 쓴다(`app/api/archive/file/route.ts:15`).
...
### 4.4 기존 흐름 조치 권고 (대표 결정 필요)

3절에서 본 것처럼 결정 12 범주의 데이터는 이미 나가고 있다. 아래 조치를 B3·A5 착수 전에 필수로 둘지, 후속으로 둘지는 대표가 정한다. 파일을 고칠 때는 병렬 레인의 파일 소유 순서를 따른다(`docs/GROWTH-PLAN.ko.md:227-233`).

1. ① 회의 입력을 허용 목록으로 바꾼다. `reviewNote`, `editStats`, `aiSource`, `aiSourceId`, `complianceHold` 발췌를 뺀다. 대상은 `originalArtifacts`와 품질 단계의 `candidateArtifacts` 둘 다다. `candidateArtifacts`에는 본문 길이 상한도 둔다(`lib/meeting-execution.ts:41,45`).
2. ② 모든 모델 입력에서 `orderRefs`를 뺀다(`lib/store-context.ts:13`).
3. ③ 자유 텍스트에 DP-3 검사를 넣는다. 검토 메모, 업로드 추출문, 담당자 필드부터 시작한다.
4. ④ 담당자 필드(`plan.owner`, `assignee`)는 모델 입력에서 빼거나 역할명으로 바꾼다.
5. ⑤ 브랜드 입력을 허용 목록으로 바꾼다. 조사·학습·제작 모두 정체성 필드(`aiBrand.identity`: 이름·약칭·업종·색·톤·`audience`·`constraints`)만 기본으로 보낸다. 소개·메모(`brandIntro`)와 의뢰 정보(`intake`)는 빼거나, DP-3 검사를 통과한 경우에만 보낸다. `aiBrand`는 `id`·`bg`만 빼므로 지금 수준으로 맞추는 것만으로는 최소화 효과가 거의 없다(`lib/ai-context.ts:16-19`). `audience`·`constraints`도 자유 텍스트(각 5,000자)라 DP-3 검사 대상에 넣는다. 조사가 `intake`의 웹사이트·SNS 링크를 과업에 필요로 하는지는 확인 필요이며, 필요하면 필드 단위로 고른다.
6. ⑥ OpenAI 직접 경로의 `store:false` 가능 여부를 검토하고, metadata의 소유자 식별자를 가명 키로 바꾸는 방안을 검토한다. 복구가 `metadata.agency_job_id`를 대조하므로 `lib/role-execution.ts:112`도 함께 바꿔야 한다.
7. ⑦ 검토 메모 입력란과 자료 업로드 화면에 개인정보 입력 금지 안내를 둔다.
8. ⑧ 조사 지시문에 리뷰·댓글 작성자의 식별정보를 빼라는 문구를 더한다(`lib/deep-research-server.ts:21-24`).
9. ⑨ 조사·학습 제출 원문, 학습 응답, 자료·R2 원본의 보존 기한과 파기 경로를 정한다(`lib/record-kinds.ts`에 정책 추가).
...
| 앱 호스팅(H1, ChatGPT Sites·Cloudflare D1·R2): 확인 필요(OpenAI의 Sites 운영 범위) | 확인 필요 | 확인 필요 | Cloudflare(D1·R2) 재수탁 여부 확인 필요 | 확인 필요 | 앱을 쓰는 동안 계속 저장 | 앱에 저장되는 모든 기록(업로드 원본 포함) | 모든 범주(업로드 내용에 제한이 없다) | 서비스 제공과 저장 | 기록 종류별 정책을 따른다(`lib/record-kinds.ts`). 자료·원본·조사 원문의 파기 경로는 확인 필요 | 확인 필요(서비스 이용의 전제다) | 서비스를 쓸 수 없다 |
...
- [ ] 6절 법률 검토 주체를 지정했고 검토 결과를 기록했다. 그 결과가 해당 기능의 입력을 허용(또는 조건부 허용)했고, 조건은 4.3 DP 규칙에 반영했다(B3는 6절 11번, A5는 6절 10번). 허용하지 않으면 해당 기능(B3-2 또는 A5)은 not_run으로 기록하고 이유를 남긴다.
...
- [ ] 4.4 기존 흐름 조치 ①~⑨를 각각 착수 전 필수로 둘지 후속으로 둘지 대표가 정했다.
- [ ] 4.4에서 착수 전 필수로 정한 조치가 merged이고 관련 테스트가 passed다.
...
| 19 | 조사·학습 제출 원문, 학습 응답, 자료·R2 원본의 파기 경로가 정말 없는지 | 파기 요청 대응 | `lib/record-kinds.ts:28,51,54`. R2 삭제는 업로드 실패 되돌리기뿐이다(`app/api/archive/file/route.ts:15`) | 삭제 경로 전수 검색. 이번에는 records 삭제 정책과 `runtime.BUCKET.delete` 사용처만 봤다 |
...
| 24 | 법률 검토 주체 | 결정 12의 착수 조건 | `docs/GROWTH-PLAN.ko.md:198` | 대표 지정(6절) |
