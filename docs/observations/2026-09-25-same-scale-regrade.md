...
품질 수정 v1 뒤 결과에 남은 규제 block 1건과 warn 9건은 원문을 재현해 보니 모두 **측정 도구 오탐**이었다. 이 오탐은 PR `fix/compliance-failure-label`(사전 `compliance-lexicon-2026-09-25.2`, `+compound-labels`)이 고친다. 게시 뒤 다시 재채점하면 수정 v1 뒤 결과는 결함 0개 산출물 11/11이 될 것으로 예상했다.

**확인(real, 2026-09-25 05:39 UTC 경, 64e51e8·Sites 버전 30 게시 뒤 재채점, 사전 `.4`)**

- 수정 v1 뒤 결함 0개 산출물 11/11(통과 81/0, 예방 fail 0, block 0)을 확인했다.
- warn은 9→4다. 남은 4건('온라인 판매' 명사 용법, 한정 조건 CTA, 확인 뒤 고르는 CTA 선택지 나열)도 원문 재현상 오탐이다. 사전 `.5`가 고친다.
- 기준선 v1의 block 1(ODA cmo)은 `### 수정·중단 조건` 목록이라 #95 뒤 0이 됐다.
- 상세: [64e51e8 게시 기록](../releases/2026-09-25-64e51e8.md).
...
1. 측정 수정 PR 병합 → 다음 묶음 게시 뒤 네 run을 다시 `regrade_run`한다(토큰 0). 수정 v1 뒤 결함 0개 산출물 11/11을 확인한다. → 확인했다(64e51e8, 위).
