...
| 레인 | 세션 | 맡는 일 | 게시 |
|---|---|---|---|
| **A** 제품 기능 | Claude A 세션 | 성장 계획 A·B 트랙 제품 기능(`docs/GROWTH-PLAN.ko.md`). A3·A6 마무리, B4 2부, B3, 조건부 A5(A8은 레인 D) | **게시 담당(단독)** |
...
| **D** 고객 보고 | Claude 보고 세션 | A8 고객 보고서·사실 지식 팩(LLM 0). 대표 현황 보고 | 하지 않음. 게시 대기열에 적는다 |
...
2. (A8은 레인 D로 옮겼다. 화면은 PR 5 범위로 남는다.)
...
### 레인 D (Claude 보고 세션)
1. A8 고객 보고서·사실 지식 팩(LLM 0). 선행 A4·B2·F2는 `merged`다. 브랜드·지점·기간 보고서와 확정 사실 팩을 결정론 집계로 낸다. 지표 정의는 `data-truth-9`와 공유하고, 화면은 PR 5(레인 A) 범위로 남긴다.
2. 대표 현황 보고(저장소 밖 산출물). 제품 코드·게시는 하지 않는다.
- 새 records kind가 필요하면 `lib/record-kinds.ts` 끝 고정 묶음 앞에 레인 D 항목만 붙인다(공유 파일 표).
- 공유 링크(`ux-1` 뒤 검토)는 이 레인 범위가 아니다.
