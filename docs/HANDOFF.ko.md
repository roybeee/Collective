...
## 역할 분리
세션별 레인(누가 무엇을 만들고 누가 게시하는지)은 [LANES](LANES.ko.md)가 정본이다. 게시는 레인 A만 요청한다. 아래 표는 사람과 도구 사이의 역할이다.
...
## 다음 작업
레인별 개발 순서는 [LANES](LANES.ko.md#레인별-개발-순서)에 있다. 이 문서에는 다음 작업을 따로 적지 않는다(중복 방지). 레인 도입 전 목록은 git 이력(`c2ece2a` 이전 HANDOFF)에 있다.
