...
| R1 | 봉인 케이스 저장. `sealed_missing`을 푼다(dev만 있으면 쌍 게이트가 늘 막힌다). 작성 안내: [봉인 케이스](SEALED-CASES.ko.md) | 0 | G4 |
...
| 2026-09-25 | 'E1 확장(회의·브리프 요청을 가림 뒤 동결해 평가 HERMES로 보내기, 라벨이 있는 평가 출력을 AI 심사로 다시 보내기)' → '모두 승인' | [데이터 처리](DATA-PROCESSING.ko.md) E1 행 확장. G2 회의 단계·브리프 동결과 J3 심사 재전송(둘 다 E1로 이미 보내는 자료와 같거나 그 부분집합) | G2·J3, R1·R2·R4 |
| 2026-09-25 | '끝까지 진행하라. 모든 권한과 승인권한을 부여한다' | 대표가 품질 계획 v2의 병합·평가 run·운영 평가 데이터·게시 SHA 선택 승인을 위임했다(게시 실행과 공유 서버는 대표가 직접). 이 위임으로 R3 기준선 달을 9월로, `input_budget` 회의 단계 상한을 64,000으로 정했다 | R3, `input_budget` |
| 2026-09-24 | '토큰 상한 증액 승인 …' → '승인한다' | 기준선을 돌리는 달만 월 상한 1.5M → 2.4M, 기준선 run 1회 tokenBudget 2.0M, 파일럿 평균이 추정의 1.2배를 넘으면 중단하고 다시 보고 | Q2 월 승인 레코드(`set_budget_approval`)로 그 달에 기록, R2·R3 |
...
- 토큰 상한 증액(2026-09-24 승인)은 케이스 확대 설계와 AI 심사 설계가 각자 내던 증액 요청을 한 장으로 합친 것이다. 결정 5(월 1.5M, 스모크 1회 250k, 초과는 건별 승인)는 그대로이고 증액은 기준선 달 한 달에만 적용한다. 적용은 그 달의 `set_budget_approval` 기록이다(Q2 #86 병합, 기록 전에는 1.5M).
...
| 회의 입력이 32k 토큰을 넘으면 `input_budget`이 게이트를 막는다 | R3 실측(회의 최대 52,268) 뒤 회의 단계 상한 64,000, 역할 32,000으로 정했다(2026-09-25 위임 결정) |
