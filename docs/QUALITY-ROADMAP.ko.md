...
| 요청 | 내용 | 반영 |
|---|---|---|
| [데이터 처리](DATA-PROCESSING.ko.md) E1 행 확장 | 회의 단계·브리프 요청 동결(G2, 운영과 같은 가림 뒤)과 평가 출력의 AI 심사 재전송(J3, E1로 이미 보낸 자료의 부분집합). 문서에는 적었고 코드는 병합됐지만 게시·실행 전이라 실제 전송은 0건이다 | 승인 전에는 회의·브리프 캡처·실행과 심사 run을 시작하지 않는다 |
...
- [데이터 처리](DATA-PROCESSING.ko.md) E1 행 확장(회의·브리프 요청 동결, 평가 출력 재전송): 위 '승인 요청(대기)'에 올렸다.
