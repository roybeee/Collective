...
- (해소, PR 4a-2) 보관과 동시에 들어온 새 AI 실행의 경합. 보관 중 새 AI 실행 차단(역할 실행 `POST /api/run` start, 팀 회의 `POST /api/meetings` start·retry_failed, 캠페인 HERMES 초안 `POST /api/brief` start + `campaignId`)은 PR 5d에서 라우트의 잠금 밖 사전 검사(`assertCampaignNotArchived`, `lib/campaign-archive.ts`)뿐이라, 보관 요청과 동시에 들어온 시작 1건이 보관된 캠페인에서 작업을 만들 수 있었다. 이제 실행 파일이 소유자 잠금을 잡고 캠페인을 읽은 직후 다시 검사한다(`lib/role-execution.ts` start, `lib/meeting-execution.ts` start·retry_failed, `lib/brief-execution.ts` start + `campaignId`). 보관도 같은 소유자 잠금 안에서 기록하므로, 사전 검사를 통과한 시작이라도 보관이 먼저 잠금을 잡았으면 409(`보관된 캠페인입니다. 보관 해제 후 다시 시도하세요.`)로 끝나고 작업·제출·기록이 생기지 않는다. 라우트 사전 검사는 빠른 거절용으로 남긴다. 이미 시작된 작업의 복구(`recover`)·취소(`cancel`)·조회는 보관 중에도 막지 않는다. 확인: `tests/execution-guards.test.mjs`(시작이 잠금을 잡는 순간 보관을 기록해 경합을 재현한다).
- 연속 실행 시작·진행과 발행 승인·접수의 보관 검사도 잠금 안에서 한다(`lib/campaign-sequence.ts`, `lib/execution-server.ts`). 이 경로에는 PR 5d부터 위 경합이 없었다.
