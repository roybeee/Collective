...
`checkCompliance(text,{facts})`가 문장 단위로 8개 범주를 점검하고 `{version,issues,notice}`를 반환한다. 현재 사전은 `compliance-lexicon-2026-09-25.7`이다. `.7`은 R3 기준선 MAPDAL 회의 실측에 따라 두 가지를 더했다. 확정 가격 자리('[확정 가격]', '[판매가]', '[가격·통화]')가 든 카피 구성의 CTA는 가격 표시가 있는 것으로 본다('[가격 확인 필요]'·'[가격 문의]'·'[가격 협의]'는 아니다). 퍼널 단계를 셋 이상 나열한 문장('유효 세션·상품 조회·장바구니·결제 시작', '주문·조회·장바구니 자료')은 측정 문장이다. `.6`은 R3 기준선 실측에 따라 세 가지를 고쳤다.
  ...
  - 위 `+r3-negation` 공용 부정(확정 뒤 승인 보류 등)을 쓴다.
