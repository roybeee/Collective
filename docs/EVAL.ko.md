...
- `+contract-read`는 운영 계약 읽기(`lib/role-output.ts`)를 고쳐 렌더본이 바뀐다(R3 기준선 계약 형식 실패 2건 실측).
  ...
  - 그래서 두 결함이 있던 출력은 이제 렌더본으로 채점한다. 이전에는 원문 JSON으로 채점했다.
- `+local-rerun`은 2026-09-25 fa9da73 로컬 채널 재실행(S2·S6·S8 총괄·전략)에서 나온 오탐 네 가지를 고쳤다.
  - 대상을 쓰려는 조건('… ‘당일 전량 소진’ 표현을 사용하려는 경우', 수정·중단 조건)은 사용이 아니다. '비가 오는 경우 ‘…’ 문구를 넣는다'처럼 조건이 대상 앞에 있으면 계속 사용이다.
  - '사용 여부'('AI 생성·합성 소재 사용 여부')는 '여부'다.
  - 닫는 따옴표 뒤 인용 조사 '고'는 절 경계가 아니다('‘체험 가격이 2만 원이다’, ‘가장 인기 있다’고 광고하지 않는다'). '‘가장 인기 있다’고 광고한다'는 계속 사용이다.
  - 전략의 '포기하는 것:' 칸 아래 목록은 금지 목록이다. 목록이 끝난 뒤 카피에 쓰면 사용이다.
...
`.12`는 fa9da73 게시 뒤 R3 재채점에 남은 warn을 재현해 고쳤다.
  - '구매하기·주문하기·결제하기' 뒤가 '어려움·전에·위해'인 서술('비교·주문하기 어려움')과 '온라인 판매처'는 구매 유도가 아니다.
  - 확인 뒤 삽입하는 CTA('… 확인 후에만 ‘구매하기’를 삽입')는 보류다.
  - 검수가 남은 CTA를 지적하는 문장('‘구매하기’ 제안이 … 남아 있어')은 사용이 아니다.
  - '지금 주문하기, 쉽고 빠르게', '‘구매하기’ 문구가 들어 있는 배너를 게시한다'는 계속 잡는다.

`.11`은 121791b 게시 뒤 R3 재채점에 남은 MAPDAL warn을 재현해 고쳤다.
  - '장바구니'는 담기 동작('장바구니 담기·장바구니에 담·추가·버튼')만 구매 유도로 본다. '장바구니 이후 이탈', '상품 상세·장바구니·결제·주문 확인 흐름' 같은 퍼널 단계 명사는 아니다.
  - 확인·확정 뒤 검토·결정하는 CTA('확인 후에만 “구매하기”를 검토', '가격 확인 후 ‘구매하기’로 결정')는 보류다. 대괄호 자리 표시 안의 줄표·쉼표는 절 경계가 아니다('[가격 확정 후 검토 — 구매하기]').
  - '가격 확인 후 … 지금 구매하세요'와 '장바구니에 담고 바로 결제하세요'는 계속 잡는다.
  - 조건 없이 `[가격 확인 필요]`만 붙인 'CTA: “구매하기”'는 품질 수정 v1 결정대로 경고다.

`.10`은 `artist_rights_unconfirmed`에서 사용 동사 뒤가 '시간·일정·채널·위치' 같은 명사인 경우('… 배우, 조명, CTA, 랜딩 페이지, 게시 시간은 고정한다')를 사용으로 보지 않는다(R3 기준선 S8 실측). `.9`는 광고·협찬 표시 계획의 인정 범위를 넓혔다. '… 발생하면 게시 문안에 해당 광고 표시를 추가합니다', 체크리스트 '광고·협찬·유료 게시라면 해당 표시를 문안에 넣었는가'가 해당된다(R3 기준선 S7 실측). '광고 표시는 넣지 않는다'는 계속 표기 누락이다. `.8`은 검수 책임표의 'AI 소재·광고 표시'처럼 AI 소재와 표시를 함께 적은 것을 표시 조치로 본다(R3 기준선 S7 실측, 'AI 소재 이미지로 배너를 만든다'는 계속 잡는다). `.7`은 R3 기준선 MAPDAL 회의 실측에 따라 두 가지를 더했다. 확정 가격 자리('[확정 가격]', '[판매가]', '[가격·통화]')가 든 카피 구성의 CTA는 가격 표시가 있는 것으로 본다('[가격 확인 필요]'·'[가격 문의]'·'[가격 협의]'는 아니다). 퍼널 단계를 셋 이상 나열한 문장('유효 세션·상품 조회·장바구니·결제 시작', '주문·조회·장바구니 자료')은 측정 문장이다. `.6`은 R3 기준선 실측에 따라 세 가지를 고쳤다.
  - '유료 광고비'(예산 칸, '유료 광고비 0원으로 우선 설계')는 협찬 표기 대상이 아니다. 광고비를 받는 대가는 계속 잡는다.
  - '다음 상황에서는 … 중단한다' 리드 아래 목록은 중단 조건 목록이라 비운다. 목록 뒤 문장과 중단 리드가 아닌 목록은 그대로 본다.
  - 위 `+r3-measure` 공용 부정(확정 뒤 승인 보류 등)을 쓴다.

`.5`는 `price_missing`·`terms_missing`에서 세 가지를 고쳤다. '온라인 판매·구매' 뒤에 조사·명사가 붙은 명사 용법('목표는 온라인 판매이며', '온라인 구매와의 관계', '온라인 판매 재고')은 구매 유도 매치가 아니다. '…할 수 있을 때만 “구매하기”'처럼 '때만·경우에만'으로 한정한 조건을 보류로 본다. 인용 문구 나열('“상품 선택하기”, “구매하기” 중 하나') 사이 쉼표는 절 경계가 아니라 앞 조건('확인한 뒤')이 나열 전체에 걸린다. 2026-09-25 64e51e8 게시 뒤 재채점에서 MAPDAL 수정 v1 뒤 결과에 남은 warn 4건을 재현해 고쳤다. '온라인 판매 시작!', '필요할 때 지금 구매하세요', '“오픈 기념”, “지금 구매하세요”'는 계속 잡는다. `.4`는 사전 항목은 그대로이고 공용 부정이 '-지는·지도 않'을 부정으로 본다(위 `+critique-clause`). `.3`은 사전 항목은 그대로이고 공용 부정 판별에 표현 부재 서술을 더했다(위 `+absent-expr`, 효능 주장이 수용 기준의 '… 표현이 없고'로만 나오면 면제, 꾸밈말 붙은 수사는 계속 잡음). `.2`는 `.1`(측정 v2)에 두 가지를 더했다: 합성 판정 라벨 표 열을 규칙 칸으로 보고(위 `+compound-labels`), `price_missing`·`terms_missing`에서 퍼널·분석 이벤트 측정 문장(퍼널 화살표·단계별 이탈·snake_case 이벤트 이름·세션 ID·조회수·'유료 주문과 구분')과 조건을 앞에 둔 CTA('확인된 경우 “구매하기”', '정상 작동한 뒤: “구매하기”')를 면제한다(2026-09-25 MAPDAL 재채점 오탐 재현). 조건 없이 `[가격 확인 필요]` 표시만 붙인 구매 문구는 품질 수정 v1 결정대로 경고를 남긴다. 카피 안 화살표·'오픈 이벤트'·블로그·다른 문장의 조건·'확정된 가격으로 구매'는 계속 잡는다. 품질 기준선 v1과 품질 수정 v1 뒤 재평가는 이전 사전 `compliance-lexicon-2026-09-23.2`로 채점했다. 사전·판정이 바뀐 뒤 이전 run과 비교하려면 두 run을 모두 재채점한다([같은 저울 재채점](#6-같은-저울-재채점regrade_run)). 어휘 사전(`lib/graders/compliance-lexicon.ts`)은 버전(`compliance-lexicon-YYYY-MM-DD.N`)과 공식 출처 URL(국가법령정보센터)을 가진다. 판정 로직과 사전을 나눠 사전만 개정할 수 있다.
