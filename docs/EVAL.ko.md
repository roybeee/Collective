...
- 채점 버전: `GRADERS_VERSION='failure-types-v1+normalized+measure-v2+g3+compound-labels+absent-expr+critique-clause+meeting-normalized'`(`lib/graders/index.ts`). `+meeting-normalized`는 회의 단계를 운영 정규화본으로 채점하고 원문 경로 노출을 `prevention`으로 둔다(아래 `prevention`). `+critique-clause`는 `revisit_cohort_definition`에서 산식(÷·나눈·비율로 정의 등)이 재방문율·재구매율이 든 절 안에 있을 때만 정의로 본다. 앞 절의 다른 지표 산식('카드 회수율은 … ÷ …로 정의하고, 재방문율과 섞지 마세요'), 'X는 재방문율이 아니라 Y', '재방문율을 계산(산출)할 수 없다'는 정의가 아니다. 공용 부정은 '-지는·지도 않'('직접 사용되지는 않았으나')도 부정으로 본다. 2026-09-25 파일럿 1(S2 회의) 토론·품질 검토에서 상류 산식을 비판한 문장이 재방문율 정의로, 거절 사실을 쓰지 말라는 검토 문장이 사용으로 잡힌 오탐을 고쳤다. `+absent-expr`는 표현 부재 서술('… 할인·무료·보장 표현이 없고', '… 문구가 포함되지 않는다', '… 문구가 들어가지 않는다')을 사용 배제로 본다. 머리 명사가 표현·문구·카피·문안·단어·어휘·문장·언급·예시일 때만이고, 명사 앞에 관형형 꾸밈말이 붙은 광고 수사('더 이상의 표현이 없습니다', '이보다 좋은 문구가 없다')와 '부작용이 없습니다'·'…이 있다'는 아니다. 2026-09-25 파일럿 1(S2 회의) 합의 단계의 수용 기준 문장이 거절 사실 사용(`fact_conflict`)으로 잡힌 오탐을 고쳤다. `+compound-labels`는 판정어를 가운뎃점·빗금으로 이은 합성 라벨('탈락·수정 기준', '실패/중단 조건')의 표 열·제목을 규칙 칸으로 본다(모든 판정어가 실패 판정이거나 곁말(수정·반려·보완·재작업·보류)이고 실패 판정이 하나 이상일 때만, '합격·수정 기준'·'수정 기준' 단독은 아니다). 2026-09-25 같은 저울 재채점에서 ODA 전략 산출물의 평가표 '탈락·수정 기준' 열이 가짜 후기 block으로 잡힌 오탐을 고쳤다([관찰](observations/2026-09-25-same-scale-regrade.md)). `+g3`는 기존 13종 결과는 그대로 두고 회의·브리프 채점기 6종(`KIND_GRADERS`, 적용 종류 밖이면 not_applicable)과 업종 사전 확장을 더했다. 사전 확장 때문에 업종이 지정된 케이스의 `industry_metric_leak` 결과가 바뀔 수 있어 버전을 올렸다. `failure-types-v1`은 정규화 전 렌더본을 채점했다(품질 기준선 v1). `failure-types-v1+normalized`는 품질 수정 v1(#80)부터 측정 v2 전까지의 채점이다(품질 수정 v1 뒤 재평가). `+measure-v2`는 13종은 같고 부정·규칙 문장 판정과 `unsupported_claim_term`·`revisit_cohort_definition` 판정을 고친 측정 도구 v2다. 판정을 바꾸면 이 값을 올린다(같은 저울 재채점의 버전 검사가 이 값에 기댄다). 서버 평가 결과(`eval_run.results[]`)에 `gradersVersion`을 남기며, 이 값이 없는 결과는 `failure-types-v1`이다.
...
- `prevention`: 원 JSON을 정규화 전 렌더본으로 다시 만들어 `heading_nesting`·`internal_id_exposure`만 돌린 판정(`runPreventionGraders`). 정규화는 두 채점기가 잡는 것만 바꾸므로, 정규화가 바꾼 건이 있으면 채점기가 원문에서 못 보는 경우(품질 JSON의 `\n` 이스케이프 바로 뒤 경로 등)도 fail이다. 저장 본문(`text`) 항목은 원문이 없어 빈 목록이다. 회의 단계는 운영처럼 정규화본(`lib/meetings.ts` `scrubMeetingOutput`: 내부 식별자·입력 스키마 경로 → 한국어 라벨, 라벨은 동결 회의 기록의 `meetingLabels`)으로 채점하고, 원문의 `internal_id_exposure`만 `prevention`에 둔다(`+meeting-normalized`, R3 기준선 2026-09-25 MAPDAL 재검토 원문의 `evidence.facts.confirmed`). 브리프는 원 JSON 그대로 채점하므로 `prevention`이 빈 목록이다.
...
`checkCompliance(text,{facts})`가 문장 단위로 8개 범주를 점검하고 `{version,issues,notice}`를 반환한다. 현재 사전은 `compliance-lexicon-2026-09-25.5`다. `.5`는 `price_missing`·`terms_missing`에서 세 가지를 고쳤다. '온라인 판매·구매' 뒤에 조사·명사가 붙은 명사 용법('목표는 온라인 판매이며', '온라인 구매와의 관계', '온라인 판매 재고')은 구매 유도 매치가 아니다. '…할 수 있을 때만 “구매하기”'처럼 '때만·경우에만'으로 한정한 조건을 보류로 본다. 인용 문구 나열('“상품 선택하기”, “구매하기” 중 하나') 사이 쉼표는 절 경계가 아니라 앞 조건('확인한 뒤')이 나열 전체에 걸린다. 2026-09-25 64e51e8 게시 뒤 재채점에서 MAPDAL 수정 v1 뒤 결과에 남은 warn 4건을 재현해 고쳤다. '온라인 판매 시작!', '필요할 때 지금 구매하세요', '“오픈 기념”, “지금 구매하세요”'는 계속 잡는다. `.4`는 사전 항목은 그대로이고 공용 부정이 '-지는·지도 않'을 부정으로 본다(위 `+critique-clause`). `.3`은 사전 항목은 그대로이고 공용 부정 판별에 표현 부재 서술을 더했다(위 `+absent-expr`, 효능 주장이 수용 기준의 '… 표현이 없고'로만 나오면 면제, 꾸밈말 붙은 수사는 계속 잡음). `.2`는 `.1`(측정 v2)에 두 가지를 더했다: 합성 판정 라벨 표 열을 규칙 칸으로 보고(위 `+compound-labels`), `price_missing`·`terms_missing`에서 퍼널·분석 이벤트 측정 문장(퍼널 화살표·단계별 이탈·snake_case 이벤트 이름·세션 ID·조회수·'유료 주문과 구분')과 조건을 앞에 둔 CTA('확인된 경우 “구매하기”', '정상 작동한 뒤: “구매하기”')를 면제한다(2026-09-25 MAPDAL 재채점 오탐 재현). 조건 없이 `[가격 확인 필요]` 표시만 붙인 구매 문구는 품질 수정 v1 결정대로 경고를 남긴다. 카피 안 화살표·'오픈 이벤트'·블로그·다른 문장의 조건·'확정된 가격으로 구매'는 계속 잡는다. 품질 기준선 v1과 품질 수정 v1 뒤 재평가는 이전 사전 `compliance-lexicon-2026-09-23.2`로 채점했다. 사전·판정이 바뀐 뒤 이전 run과 비교하려면 두 run을 모두 재채점한다([같은 저울 재채점](#6-같은-저울-재채점regrade_run)). 어휘 사전(`lib/graders/compliance-lexicon.ts`)은 버전(`compliance-lexicon-YYYY-MM-DD.N`)과 공식 출처 URL(국가법령정보센터)을 가진다. 판정 로직과 사전을 나눠 사전만 개정할 수 있다.
...
- 회의 단계(`meeting_step`, G2): 저장 요청은 `{meeting, stepId, storeAllow}`다(`lib/eval-freeze.ts`).
  - 자르기: 대상 단계 직전의 회의 기록이다. 앞 단계는 완료 출력 그대로, 대상 단계는 출력·교정 없이 대기, 뒤 단계는 없다. 교정 재시도(`correction`·`attempt`)는 떼어 첫 제출(attempt 0)을 재현한다.
  - 줄이기: 조립이 읽는 필드만 둔다. 단계는 `id·role·phase·status·output·task`, 작업물은 회의 입력 허용 목록(`MEETING_ARTIFACT_FIELDS`), 브랜드는 `aiBrand`가 읽는 필드다. 실행 메타(`providerId`·`raw`·시도 기록·가림 기록), 검토 메모·사람 수정 표시, 브랜드 의뢰 정보·`bg`는 저장하지 않는다. 담당자 실명은 조립과 같은 자리표시로 바꾼다.
  - 가림: 운영 가림 경로(`MEETING_MASK_PATHS`)마다 대응하는 원자료 자리(`MEETING_FREEZE_MASK_PATHS`, 개선본 출력은 제목·본문)를 운영과 같은 허용 값으로 가린다. 조립이 다시 가려도 자리표시는 다시 탐지되지 않아 결과가 같다(두 번 가린 결과 = 한 번 가린 결과). 운영 경로에 대응이 없는 새 경로가 생기면 모듈 로드가 실패해 테스트가 잡는다. 운영도 가리지 않는 모델 출력(발언·합의·재검토)은 그대로 둔다.
  - 채점 항목: 발언은 기존 `discussion`(발언 JSON, 앞선 발언 역할)이다. 합의·개선본·재검토는 `meeting_step`이다(단계, 원 JSON, 개선본의 원 작업물 8,000자, 재검토의 합의 과제 담당, 실무 스킬 회의면 계약). 본문은 개선본이면 `content`, 합의·재검토면 코드 필드와 되묻는 질문을 뺀 문장이다. 브랜드 소개(`brand_intro_as_fact`)와 심은 결함(`seeded_defect_detection`)을 컨텍스트로 준다.
- 브리프(`brief`, G2): 저장 요청은 `lib/brief-input.ts` `BriefRequest`(`{input, context, contextDate, storeAllow}`)다. 담당자 실명 자리표시·브랜드 필드 최소화 뒤 운영 가림 경로(`BRIEF_MASK_PATHS`)의 원자료 자리(`BRIEF_FREEZE_MASK_PATHS`)를 같은 허용 값으로 가린다. 채점 본문은 계획이 되는 summary·제안 값이다(questions·assumptions 제외). 사용자 입력과 기준일(`briefInput`)에 적힌 가격·날짜는 단정으로 보지 않는다. 담당은 `brief`이고 쌍 평가 대상이 아니다.
- 캡처 드리프트 판정(`captureCheck`, `lib/eval-capture.ts`): 지금 코드로 다시 조립한 제출을 운영이 그때 저장·전송한 제출(`hermes_submission`)과 비교한다. 지시문·입력만 보고 무작위 `session_id`는 뺀다. 회의 단계는 첫 제출과 비교한다.

| `submission` | 뜻 | 경보 |
|---|---|---|
| `identical` | 같다 | 아니오 |
| `no_submission` | 저장된 제출이 없다 | 아니오 |
| `code_changed` | 회의 당시 실무 스킬 버전(`PRACTICE_VERSION`)이 지금과 다르다. 브리프는 지시문이 다르다 | 아니오 |
| `store_allow_changed` | 회의 단계의 가림 기록이 그때와 다르다. 지점 허용 값은 지금 값으로 다시 읽는다 | 아니오 |
| `context_changed` | 브리프 원자료(다른 캠페인·성과·학습 규칙)를 지금 DB로 다시 읽어 입력이 다르다 | 아니오 |
