...
`checkCompliance(text,{facts})`가 문장 단위로 8개 범주를 점검하고 `{version,issues,notice}`를 반환한다. 현재 사전은 `compliance-lexicon-2026-09-25.11`이다. `.11`은 121791b 게시 뒤 R3 재채점에 남은 MAPDAL warn을 재현해 고쳤다.
