...
| `noteLength` | 검토 메모(작업물 수정 요청·승인 메모) 글자 수. 메모가 없으면 필드가 없다. 메모 원문은 고객 원문이 들어갈 수 있어 여기에 복사하지 않는다(원문은 캠페인과 함께 지워지는 작업물 `reviewNote`·이벤트에만 있다) |
| `actor` | 행위자 `{id,role}`. 캠페인을 지워도 남는 기록이라 이메일은 담지 않는다(계정 id로 찾는다) |
...
| `origin` | 작업물 판정 때의 출처(`ai`·`ai_edited`·`manual`). B1 이전에 사람이 고친 판(`origin ai`·v2 이상·`meetingId` 없음)은 `ai_edited`로 적는다 |
...
보존: B2·B3 평가 자료라 `eval_case`처럼 캠페인을 지워도 남긴다(`retain`, `data_campaign`, 삭제를 막지는 않는다). 그래서 남는 것은 사유 코드·판정·버전·계정 id·역할뿐이고 검토 메모 원문과 이메일은 처음부터 저장하지 않는다. 캠페인 삭제 영향 조회에는 보존 건수로 잡히고 대화상자에는 '사람 판정 로그 N건(사유 코드·판정만, 검토 메모 원문 없음)'으로 보인다(`lib/deletion-summary.ts`). 자료 제외 판정은 `campaignId`가 없다.
예외(F4b-2, 결정 7): 대표(소유자)가 삭제 대화상자에서 '학습 자산까지 완전 삭제'를 고르면 그 캠페인의 판정 로그(`campaignId`가 같은 행)도 지운다. 판정 로그는 캠페인을 식별할 수 있는 평가 신호이고, 결정 7은 평가 신호를 비식별로만 보관하며 대표가 완전 삭제를 고를 수 있게 하기 때문이다. '추가만 한다'는 고쳐 쓰지 않는다는 규칙이라 이 삭제와 충돌하지 않는다. 기본 삭제에서는 지금처럼 남는다. 이관 레코드에는 사람 판정을 중복해 담지 않는다(`docs/DATA-PROCESSING.ko.md` 9절).
...
| `POST /api/action` `save_artifact` | AI 작업물(`origin ai`)을 사람이 고쳐 저장하면 `origin ai_edited`, `aiSourceId`(`<원본 id>:<원본 버전>`, 채점 기록 `grading` id와 같은 형식), `aiSource`(원본 실행 버전), `editStats`(`changedSections` 바뀐 섹션 제목, `diffRatio` 0~1 줄 단위 편집 비율)를 남긴다. 다시 고쳐도 원본과 비교한다. 처음부터 직접 쓴 작업물은 `manual` 그대로다. B1 이전에 사람이 고친 판(`origin ai`·v2 이상·`meetingId` 없음, `lib/history-labels.ts` `madeBy`와 같은 기준)은 과거 사람 수정본으로 보고, 이력에서 그보다 앞선 실제 AI 판(v1 또는 회의 개선)의 가장 최근 판을 AI 원본으로 찾는다. 찾지 못하면 `aiSource` 없이 `ai_edited`다 |
...
| `POST /api/execution` `cancel`·`reconfirm` | 발행 취소 `cancelled`, 재확인(승인을 초안으로 되돌림) `returned` 1건. `reasonCodes` 선택. 판정은 상태 변경(재확인은 이벤트 포함)과 같은 `db.batch`로 쓴다. 판정 쓰기가 실패하면 상태도 바뀌지 않아 같은 버전으로 다시 시도할 수 있다 |
...
| `firstPassApprovalRates(owner)` | 역할×`skillVersion`별 1차 승인율 `{role,skillVersion,artifacts,approvedFirst,editedFirst,rate}`. AI가 만든 작업물(`ai`·`ai_edited`)마다 첫 판정을 보고, 사람이 고치지 않은 AI 판(`origin ai`)의 첫 판정이 `approved`일 때만 1차 승인이다. 사람이 먼저 고친 판의 첫 판정은 분모에만 넣고 `editedFirst`로 센다. 직접 작성(`manual`)은 뺀다(순수 계산은 `firstPassApproval`) |
| `criterionUnits(owner,campaignId?)` | B2 κ 단위 `(artifactId, version, criterion)`별 사람 판정과 AI 상태 |
| `preferencePair(owner,artifactId)` | 선호 쌍: AI 원본(`ai`)·사람 확정본(`human`, 승인했거나 사람이 고친 현재 판)·판정 이력. 직접 작성한 작업물은 `null`. B1 이전 사람 수정본은 이력의 실제 AI 판을 `ai`로 쓰고, 찾지 못하면 `ai`가 `null`이다. 조회용이며 학습에는 쓰지 않는다 |
