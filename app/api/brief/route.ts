...
export async function POST(req:Request){let owner='',lock='',pending:StoredDraft|undefined;
 try{
  owner=identity(req);secureMutation(req);const b=await body(req);const id=str(b.id,'초안',100,true);if(!/^[a-zA-Z0-9_-]{1,100}$/.test(id))throw new ApiError(400,'초안 번호가 올바르지 않습니다.');
  ...
  if(b.action==='start'){
   ...
   const trialLearning=await learningContext(owner,input);const archive=await brandArchiveContext(owner,brand.id);
  ...
  if(!['poll','recover','cancel'].includes(b.action))throw new ApiError(400,'지원하지 않는 초안 작업입니다.');
  ...
  const r=await pollHermes(cfg,next.providerId!,b.action==='cancel');
  ...
  await recordStatement(owner,'brief_draft',id,next).run();return json(publicDraft(next));
 }catch(e){
