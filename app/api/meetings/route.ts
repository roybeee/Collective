...
import {ApiError,identity,secureMutation,body,str,json,failure,database,readRecord,listRecords,recordStatement,eventStatement,connection,acquireLock,releaseLock,stamp} from '@/lib/server';
...
function writes(owner:string,m:Meeting){
 ...
 return [recordStatement(owner,'team_meeting',m.id,m,m.campaignId),database().prepare('UPDATE jobs SET status=?,error=?,tokens=?,updated_at=? WHERE owner=? AND id=?').bind(status,m.error||null,m.steps.reduce((n,s)=>n+(s.tokens||0),0),m.updatedAt,owner,jobId(owner,m))];
}
function context(m:Meeting,s:MeetingStep){
 const snapshot=m.snapshot;
 return {skillVersion:m.skillVersion,channelPractice:campaignPractice(snapshot.campaign),agenda:m.agenda,role:s.role,phase:s.phase,brand:snapshot.brand,brandArchive:snapshot.brandArchive,campaign:snapshot.campaign,trialLearning:snapshot.learning,recordedMetrics:snapshot.metrics,previousMeeting:snapshot.previous,
...
export async function POST(req:Request){let owner='',lock='',prepared:Meeting|undefined,recovering=false;
 try{
  ...
  if(!['advance','recover','cancel'].includes(b.action))throw new ApiError(400,'지원하지 않는 회의 작업입니다.');
  ...
  if(!s.providerId){
   ...
   recovering=true;prepared=m;const r=await submitHermes(owner,s.id,cfg);s.providerId=r.id;s.status='running';m.status='running';m.error=undefined;m.updatedAt=stamp();await database().batch(writes(owner,m));return json(publicMeeting(m));
  }
  const result=await pollHermes(cfg,s.providerId,m.stopRequested);
  m.updatedAt=stamp();m.error=undefined;m.status='running';s.status='running';
  if(m.stopRequested){
   if(['completed','cancelled','failed'].includes(result.status)){s.status='cancelled';m.status='cancelled'}
   await database().batch(writes(owner,m));return json(publicMeeting(m));
  }
  if(result.status==='completed'){
   ...
   s.status=result.status;m.status=result.status;m.error='담당자 작업이 종료되어 회의를 멈췄습니다. 기록을 바탕으로 후속 회의를 시작할 수 있습니다.';await database().batch(writes(owner,m));
  }else await database().batch(writes(owner,m));
  return json(publicMeeting(m));
 }catch(e){
  if(prepared){
