...
import {inspectResearchAccess,deepInstructions,parseDeepReport} from '@/lib/deep-research-server';
...
export async function GET(req:Request){try{const owner=identity(req);const cfg=await connection(owner);if(cfg.provider!=='hermes')throw new ApiError(409,'HERMES 연결이 필요합니다.');return json(await inspectResearchAccess(cfg))}catch(e){return failure(e)}}
export async function POST(req:Request){let owner='',lock='',prepared:BrandResearch|undefined,recovering=false;try{
 ...
 if(b.action==='start'){
  ...
  r={...(mode==='deep'?{protocol:DEEP_RESEARCH_VERSION,plan:defaultResearchPlan(brand),access:await inspectResearchAccess(cfg)}:{}),...(previous?{previousResearchId:previous.id}:{}),id,brandId,mode,status:'running',steps:mode==='deep'?[{id:id+'-investigation',stage:'investigation',status:'pending'}]:[...Array.from({length:Math.ceil(sources.length/30)},(_,i)=>({id:id+'-step-'+i,stage:'identity' as const,status:'pending' as const,sourceIds:sources.slice(i*30,(i+1)*30).map(s=>s.id)})),{id:id+'-diagnosis',stage:'diagnosis',status:'pending'}],model:cfg.model,stopRequested:false,createdAt:stamp(),updatedAt:stamp(),tokens:0,snapshot:{brand,observations:(await listRecords<ChannelObservation>(owner,'brand_observation',brandId)).slice(0,12)}};
  await database().batch([recordStatement(owner,'brand_research',r.id,r,brandId),database().prepare('INSERT INTO jobs(id,owner,campaign_id,role,status,model,campaign_version,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)').bind(jobId(owner,id),owner,'brand:'+brandId,'brand_research','in_progress',cfg.model,1,r.createdAt,r.updatedAt)]);if(mode!=='deep')return json(publicResearch(r));
 }
 else r=await readRecord<BrandResearch>(owner,'brand_research',id);
 ...
 const result=await pollHermes(cfg,step.providerId,r.stopRequested);r.updatedAt=stamp();r.error=result.needsApproval?'HERMES에서 도구 사용 승인을 기다리고 있습니다. 연결된 HERMES에서 요청 내용을 확인하세요.':undefined;r.status='running';step.status='running';
