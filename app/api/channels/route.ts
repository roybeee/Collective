...
export async function GET(req:Request){try{return json(await channelStatus(await identity(req)))}catch(e){return failure(e)}}
export async function POST(req:Request){let owner='',lock='';try{
 owner=await identity(req);secureMutation(req);await requireAdmin(req);const b=await body(req);lock=await acquireLock(owner);
