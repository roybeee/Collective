...
import type {ViralCase} from '@/lib/learning';
import type {Brand} from '@/lib/agency';
type Task={id:string;kind:'analysis'|'discovery';brandId:string;caseId?:string;query?:string};
...
export async function POST(req:Request){let owner='',token='',started='',submitted=false;try{
 ...
 const id=str(b.id,'학습 실행',200,true),job=await db.prepare("SELECT * FROM jobs WHERE id=? AND owner=? AND role IN ('viral_analysis','viral_discovery')").bind(id,owner).first<Job>();if(!job)throw new ApiError(404,'학습 작업을 찾을 수 없습니다.');
