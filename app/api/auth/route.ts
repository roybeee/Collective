...
import {authDb,authMode,authPrincipal,cleanupAuthRecords,hashToken,publicUser,requirePrincipal,sessionCookie,sessionToken} from '../../../lib/auth-session';
import {authBody,authJson,clearRateLimit,normalizeEmail,rateLimit} from '../../../lib/auth-request';
...
export async function POST(req:Request){try{
 ...
 const account=email||'session:'+await hashToken(sessionToken(req)||'anonymous');
 await rateLimit(req,account);
 ...
 // 성공하면 계정 제한을 초기화한다. 인증은 이미 반영됐으므로 초기화·정리가 실패해도 결과와 쿠키는 그대로 돌려준다.
 // 로그인 성공 때 만료 인증 기록도 정리한다(색인된 삭제 3건).
 await clearRateLimit(req,account).catch(()=>console.error('auth_rate_clear_failed'));
 if(body.action==='login')await cleanupAuthRecords().catch(()=>console.error('auth_cleanup_failed'));
