  ...
  {!state?<p>실행 상태를 불러오고 있습니다.</p>:<>
   ...
   <section className="rounded-xl border p-4 space-y-3"><h3 className="font-semibold">3. 발행 준비·승인</h3>
    ...
    {state.publications.map(p=>{
     ...
     const blockers=p.status==='draft'?approvalBlockers({campaign,publication:p,state,factCount:facts.length,rightsConfirmed:!!rights[p.id],aiDisclosureConfirmed:!!aiChecks[p.id]}):[];
