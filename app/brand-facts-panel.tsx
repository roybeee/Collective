...
import {factCatalog,factCatalogItem,factLabel} from '@/lib/fact-catalog';
...
type SaveResult={error?:string;warnings?:string[];reviewPublications?:number|null};
...
export function BrandFactsPanel({campaign,brandId,onChanged}:{campaign?:Campaign;brandId?:string;onChanged?:()=>void}){
 ...
 const load=useCallback(async(signal?:AbortSignal)=>{
  ...
  try{
   ...
   const response=await fetch('/api/brand-facts?'+params,{signal});const data=await response.json() as {facts:(BrandFact&FactDecision)[];error?:string}&Partial<Overview>;
   if(!response.ok)throw new Error(data.error||'사실 목록을 불러오지 못했습니다.');
   if(!signal?.aborted){setFacts(data.facts.filter(f=>!f.storeId||f.storeId===storeId));setOverview(data.readiness&&data.checks&&data.stores?{readiness:data.readiness,checks:data.checks,stores:data.stores}:null);setCheckedAt(Date.now())}
 ...
 function edit(fact:BrandFact|null){
  ...
  setOtherKey(!!fact&&!factCatalogItem(fact.key));setConfirmed(false);setScopeConfirmed(false);setOpen(true);setError('');setMessage('');setAlertNote('');
 }
 ...
 async function save(event:FormEvent){
  ...
  try{
   const {response,data}=await postFacts<SaveResult>({action:'save_fact',id:editing?.id,version:editing?.version,confirmed:form.status==='confirmed'&&confirmed,data:{...form,brandId:brand,...(targetStore?{storeId:targetStore}:{}),verifiedAt:isoInput(form.verifiedAt),validUntil:isoInput(form.validUntil)}});
 ...
 return <section aria-label="브랜드 확인 사실"><div className="section-heading"><div><h3>브랜드 확인 사실</h3><p>조리 방식·가격·영업시간 등 근거를 확인한 내용을 관리합니다.</p></div><Button disabled={saving} onClick={()=>edit(null)}>사실 추가</Button></div>
  ...
  {open&&<form className="form-stack" onSubmit={save} aria-label="브랜드 사실 편집"><fieldset disabled={saving} className="form-stack"><legend>{editing?'사실 수정':'새 사실'} · {scopeName(targetStore)}</legend>
   <label className="field"><span>사실 항목</span><NativeSelect required value={otherKey?OTHER:(keyItem?.key??'')} onChange={e=>{const value=e.target.value;setOtherKey(value===OTHER);setForm({...form,key:value===OTHER?'':value});setScopeConfirmed(false)}}><NativeSelectOption value="" disabled>항목 선택</NativeSelectOption>{factCatalog.map(i=><NativeSelectOption key={i.key} value={i.key}>{i.label}{i.storeScoped?' · 지점별 항목':''}</NativeSelectOption>)}<NativeSelectOption value={OTHER}>기타(자유 입력)</NativeSelectOption></NativeSelect></label>
