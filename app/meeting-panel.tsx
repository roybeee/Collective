...
export function MeetingPanel({campaign,workspace,onUpdated,onOutputs,onConnect}:{campaign:Campaign;workspace:WorkspaceData;onUpdated:()=>Promise<void>;onOutputs:()=>void;onConnect:()=>void}){
 ...
 useEffect(()=>{
  if(paused)return;let disposed=false;
  const timer=setInterval(async()=>{const active=current.current.find(m=>m.status==='running');if(!active||pending.current||disposed)return;pending.current=true;setPolling(true);
   try{const m=await post('advance',{id:active.id});if(!disposed){merge(m);setError('');if(!meetingActive(m))await onUpdated()}}
   catch(e){if(!disposed){setError((e as Error).message);setPaused(true)}}finally{pending.current=false;if(!disposed)setPolling(false)}
 ...
 function exportMeeting(){if(!m)return;downloadText('팀-회의-'+m.id+'.md',`# 팀 회의\n\n${m.agenda}\n\n${date(m.createdAt)} · ${stateName[m.status]}\n\n`+m.steps.filter(s=>s.output).map(s=>`## ${roleName(s.role)} · ${phaseNames[s.phase]}\n\n${JSON.stringify(s.output,null,2)}`).join('\n\n'))}
 return <section className="meeting-panel">
  ...
  {!active&&<div className="meeting-composer"><Label htmlFor={'agenda-'+campaign.id}>{previous?'후속 회의 안건':'회의 안건'}</Label><Textarea id={'agenda-'+campaign.id} value={agenda} onChange={e=>{setAgenda(e.target.value);startId.current=null}} rows={3} maxLength={5000}/><div className="meeting-composer-actions"><span>1회에 최대 3개 담당 개선 · 최대 13회 AI 작업</span><Button disabled={!loaded||busy||otherRun||!agenda.trim()} onClick={()=>connected?void action('start'):onConnect()}><Play/>{connected?'팀 회의 시작':'HERMES 연결'}</Button></div>{otherRun&&<p className="subtle-note">현재 AI 작업을 완료하거나 취소한 뒤 회의를 시작할 수 있습니다.</p>}</div>}
  <p className="meeting-help">등록된 서버 작업자가 온라인이면 화면을 닫아도 회의가 이어집니다. 작업자가 연결되지 않았을 때는 이 화면을 열어두세요. 접수된 작업과 기록은 유지됩니다. 개선본은 검토 대기 상태로 저장되고 영향받는 후속 작업물은 이전 버전으로 전환됩니다.</p>
  {error&&<div className="meeting-error" role="alert"><p>{error}</p><Button variant="outline" size="sm" onClick={()=>{setError('');setPaused(false);void load().catch(e=>setError(e.message))}}><RefreshCw/>다시 확인</Button></div>}
  ...
  {m&&<><div className="meeting-session-title"><h4>{m.agenda}</h4><span>{m.steps.filter(s=>s.status==='completed').length}/{m.steps.length} 단계 완료 · 브리프 v{m.campaignVersion}</span></div><div className="meeting-roster">{roles.map(r=>{const s=m.steps.find(s=>s.phase==='discussion'&&s.role===r.id);return <div key={r.id} className={s?.status==='completed'?'done':step?.id===s?.id&&meetingActive(m)?'speaking':''}><span className="role-avatar" style={{background:r.color}}>{s?.status==='completed'?<Check size={15}/>:r.initial}</span><small>{r.name}</small></div>})}</div>
   ...
   {!meetingActive(m)&&<div className="meeting-followup">{m.error&&<p className="form-error">{m.error}</p>}<Button variant="outline" disabled={!!active} onClick={followUp}><MessageSquare/>이 회의에서 이어가기</Button><small>미해결 쟁점·검수 결과와 최신 작업물을 다음 회의에 전달합니다.</small></div>}
  </>}
