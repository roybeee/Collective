...
사용자 승인: “전체적으로 개선하라”(2026-09-23). 기준 origin/main `3f68f21`, 첫 브랜치 `chore/engineering-baseline`(PR 0).
감사 95건(confirmed 94, refuted 1)을 PR 0~6에 배정했다. 범위·배정·제외 항목은 [전체 개선 계획](docs/IMPROVEMENT-PLAN.ko.md).

- PR 0 엔지니어링 기반: CI 차단 검사, 워커 계약 테스트 간헐 실패, lint 감축·기준선 하향, 문서 정합화. 제품 동작 변경 없음.
- PR 1 보안·인증 → PR 2 AI 품질 루프 → PR 3 첫 게시 경로 → PR 4 측정·비용 → PR 5 화면·UX → PR 6 조사 서버 보안.

결정: 저장소 공개 유지·민감 정보만 제거, PNG는 앱 자체 공개 주소, 운영 게시는 각 PR 병합 뒤 별도 승인.

## 이전 계획

# 브랜드 사실부터 발행·주문까지 실행 연결

사용자 승인: “좋아 그렇게 개발 진행하자.” 브랜치 `feat/agency-execution-loop`.
최신 이메일 인증 main `1ecd72e`를 통합하며, 개발 범위와 수용 기준은 [실행 계획](docs/EXECUTION-LOOP-PLAN.ko.md)에 기록한다.
