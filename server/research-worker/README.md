...
- **네트워크**: `collective-browser-firewall.service`가 nftables `inet collective_browser`
  ...
  9333 포트(인증 없는 CDP)로의 새 연결을 root와 hermes 계정에만 허용하고, 설치 점검 때만 여는 루프백
  9334 점검 포트(역시 인증 없음)로의 새 연결은 root에만 허용한다. 브라우저 서비스는 방화벽
...
- **설치 점검(음성 테스트 포함)**: 브라우저 계정이 샌드박스를 켠 채 점검용 Chromium을 임시 루프백 점검
  포트(9334, 서비스 CDP 포트와 별도)로 띄운다. 띄우기 전에는 점검 포트를 LISTEN하는 소켓이 없어야 하고,
  띄운 뒤에는 모두 브라우저 계정 소유여야 한다(포트 선점 확인). 이어서 `collective-npm` 계정의 점검 포트
  접속이 실패해야 한다(root의 `/json/version`이 양성 대조군). root가 CDP `/json/new`로 무작위 제목
  토큰을 넣은 `data:` 페이지를 열어 `/json/list` 제목에 그 토큰이 보여야 하고(양성), 브라우저 계정이
  읽을 수 있는 `file://` 페이지(다른 토큰)는 정책으로 막혀 제목이 보이지 않아야 한다. 제목이 안 보인다는
  것만으로는 시간 초과·비정상 종료와 구분되지 않으므로 직후 새 토큰의 `data:` 페이지가 열려야(재확인)
  통과다. `file://` 페이지를 여는 `/json/new` 요청 자체가 실패해도 차단 증거로 보지 않고 멈춘다.
  Chromium 출력은 root 소유 점검 폴더의 로그로 받는다. 점검 포트가 열리지 않거나 `data:` 페이지가
  열리지 않으면 로그 끝 몇 줄을 사유로 출력한다. 끝나면 점검 브라우저를 프로세스 그룹째 끝내고 프로필·점검
  페이지·로그를 지운다. 설치기가 SIGHUP(ssh 연결 끊김)·SIGTERM으로 끝나도 이 정리를 하고, 다음 실행은
  남은 점검 브라우저(점검 프로필)를 먼저 끝낸 뒤 포트 선점을 확인한다. 이전에는 `--dump-dom`으로 읽었지만, 2026-09-24 운영 서버 첫 설치에서 Chrome for
  Testing 154의 `--dump-dom`이 끝나지 않아 샌드박스 점검이 오판으로 멈췄다. 진단으로 샌드박스·AppArmor·
  방화벽은 정상임을 확인해 CDP 방식으로 바꿨다. 점검을 통과하면 서비스를 띄워 root로 CDP 포트가 열렸는지
  확인하고(양성 대조군), 알려 준 WebSocket 주소가 루프백 9333이며 그
