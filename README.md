...
- New deep research submits one durable HERMES run before the start request returns. It includes a business-specific plan, brand/operations, customers/competitors, comparable content, critique and at most two follow-up investigations, then diagnosis. The default plan targets 15 own-brand content cases in 90 days and three alternatives; unmet targets remain explicit gaps. HERMES continues an accepted run while the screen is closed; reopening/polling validates and synchronizes the result into the archive. This is not a Site scheduler or automatic background archive ingestion. Existing four-step jobs and classification (batches of 30 plus diagnosis) still advance while the archive is open. Exact input and idempotency keys persist before submission; uncertain acknowledgements stay locked for recovery.
