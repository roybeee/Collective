...
- Research uses HERMES in four durable steps: identity/product, customer/competition, channel/content, diagnosis. Classification of provided material runs sequential batches of 30 plus a diagnosis. Each submission persists its exact input and idempotency key before calling the provider; uncertain acknowledgements remain locked for recovery. Pending cancellation never starts a new call. Research progresses while the archive is open. Actual model/reasoning settings come from HERMES; no Astra-ultra selection or ASIDE availability is asserted.
