...
Validation: `node --experimental-vm-modules tests/stores.test.mjs` uses actual handlers with transactional SQLite and mocked HERMES replies, covering ownership, version conflicts, local research, excluded evidence, campaign linkage, measurement boundaries and learning isolation. Existing archive, brief, meeting, learning, deletion and workspace suites remain applicable. Test module loading leaves graph instantiation to VM to support shared dependency graphs. Live HERMES browsing and actual marketing performance need a configured production run.
