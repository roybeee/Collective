...
- Original files use the private logical R2 `BUCKET`; D1 stores owner-scoped metadata and extracted text. PDF/DOCX/TXT/MD/CSV/JSON and PNG/JPEG/WebP are accepted, maximum 8 MB and 200 sources per brand. TXT/MD/CSV/JSON text is re-extracted on the server from the uploaded bytes (UTF-8, or UTF-16 with a BOM; other encodings such as CP949 CSV keep only the original file); PDF/DOCX text is extracted in the browser and labelled "브라우저 추출(서버 미검증)" in the source detail, list and bulk review. PDF limits are 80 pages/80,000 characters, DOCX body only, images/scans have no OCR. Admins can delete the original file (record and text are kept, download returns 404). Details: [docs/SECURITY-BOUNDARIES.ko.md](docs/SECURITY-BOUNDARIES.ko.md). Truncation and extraction failure are explicit. Downloads require ownership and force attachment/nosniff. Failed metadata writes clean up the just-uploaded object.
