// 프롬프트 레지스트리(F3a) 기준선: 레지스트리가 비어 있으면 역할·회의·바이럴 학습의 제출 본문(instructions·input)과 역할 inputHash가 기준 커밋과 바이트 동일한지 본다.
...
import {sha,seed,mockHermes,runRole,runMeeting,roleCampaign,meetingCampaign,roleIds,brand} from './helpers/prompt-seed.mjs';
...
const captured={roles,meeting:steps,learning:[{kind:'analysis',instructions:digest(a.instructions),input:digest(a.input)},{kind:'discovery',instructions:digest(d.instructions)}],providerCalls:{external:hermes.external.length}};
