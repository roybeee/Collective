...
const runs=new Map(),external=[],errors=[];let seq=0,failGradingWrite=null;
...
const graders=await load('lib/graders/index.ts'),usageRoute=await load('app/api/usage/route.ts'),registry=await load('lib/record-kinds.ts');
