...
check('both sides are graded by the same fifteen graders and report a model',()=>assert.ok(R1.results.every(x=>x.graders.length===15&&x.model==='mock-eval-model')));
...
check('pair report compares the two sides and passes the gate',()=>assert.ok(r.status===200&&r.body.gate.ok===true&&r.body.gate.cases===2&&r.body.gate.sealedCases===1&&r.body.gate.sealedRegressions.length===0&&r.body.comparison.graders.length===15&&r.body.comparison.sharedCases===2,JSON.stringify(r.body.gate)));
