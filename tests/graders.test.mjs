...
check('claim term treats caution rule names, whether-questions and not-a-fact statements as non-use',()=>{
 for(const item of [role('적용한 학습 규칙은 ‘할인 마감 문구 주의’ 버전 1이다.'),role('근거: 크리에이티브 v1 「카피 초안」, trialLearning 「할인 마감 문구 주의」.'),copy('[확인 사실: 주소/합성 원장, 2026-03-12 확인, 브랜드 범위] 학원명·교육 과정·수강료·정원·레벨 테스트의 무료 여부·오픈일·합격률·성적 향상 수치·수업 방식은 확정 사실로 확인되지 않았다.'),role('또한 ‘무료 레벨 테스트’는 브리프 v1 목표와 총괄 파트너 v1에 반복되지만 확정 사실이 아니므로, 이를 광고 문구로 바로 쓰면 주장이 사실로 승격됩니다.'),role('‘무료 레벨 테스트’는 브리프의 목표 표현에 포함되어 있으나 실제 무료 제공 조건은 별도 확인되지 않았으므로 광고 문구로 확정하지 않는다.'),role('캠페인 목표는 ‘중학생 학부모의 무료 레벨 테스트 상담 신청을 늘린다’이며 상담 신청률은 랜딩 방문 중 상담 신청 완료 비율로 정의되어 있음.')])assert.equal(status('unsupported_claim_term',item),'pass',item.text)});
check('claim term still fails claims in quotes that are not rule names and plain free offers',()=>{
 for(const item of [copy('“할인 마감 임박, 지금 신청하세요”'),copy('레벨 테스트 무료 제공, 지금 신청하세요.'),role('‘무료 레벨 테스트’는 이번 달 확정된 혜택입니다.'),copy('‘할인 주의보 해제 기념 이벤트’를 엽니다.'),role('메시지 목표는 ‘무료 레벨 테스트 신청’을 알리는 것이다.'),role('‘무료 레벨 테스트’는 확인된 혜택이라 광고 첫 줄에 쓴다.')])assert.equal(status('unsupported_claim_term',item),'fail',item.text)});
...
// 합성 라벨 열: '탈락·수정 기준'은 규칙 칸(면제), '합격·수정 기준'은 합격이 섞여 규칙 칸이 아니다(2026-09-25 실측 재현).
const compoundTable=head=>'### 크리에이티브 평가 기준\n| 평가 항목 | 합격 기준 | '+head+' |\n|---|---|---|\n| 표현 안전성 | 근거 없는 표현을 쓰지 않음 | “가장 인기 있는 K-POP 음반” 같은 순위 표현 |';
check('claim term skips a compound failure column and keeps a mixed one',()=>{assert.equal(status('unsupported_claim_term',role(compoundTable('탈락·수정 기준'))),'pass');assert.equal(status('unsupported_claim_term',role(compoundTable('실패/중단 조건'))),'pass');assert.equal(status('unsupported_claim_term',role(compoundTable('합격·수정 기준'))),'fail')});
...
// 파일럿 1(2026-09-25 S2 회의 실측): 다른 지표의 산식을 비판·구분하는 문장은 재방문율 정의가 아니다. 산식이 재방문율과 같은 절에 있어도 '재방문율이 아니라'·'계산할 수 없다'면 정의가 아니다.
const revisitCritiques=['채널 & 그로스 v1의 ‘카드 회수 수 ÷ 첫 방문 고객 수’는 고객 식별이 없으면 재방문율이 아니라 회수율에 그칩니다.','D7의 ‘카드 회수 수를 첫 방문 고객 수로 나눈다’는 제안은 카드 회수와 유료 재방문을 혼동하며, 고객 식별이 없으면 재방문율을 계산할 수 없습니다.',"카드 회수율은 '회수 카드 수 ÷ 실제 배포 카드 수'로 별도 정의하고, 재방문율과 같은 표에 섞지 마세요.",'고객 식별 없이는 재방문 고객 수 ÷ 첫 방문 고객 수로 재방문율을 산출할 수 없습니다.'];
check('revisit definition ignores critiques and other-metric formulas (pilot 1)',()=>{for(const s of revisitCritiques)assert.equal(status('revisit_cohort_definition',role(s+'\n'+long)),'not_applicable',s)});
check('revisit definition still fails a formula asserted as the revisit rate',()=>{for(const s of ['카드 회수 수 ÷ 첫 방문 고객 수가 곧 재방문율입니다.','재방문 고객 수 ÷ 첫 방문 고객 수를 재방문율로 정의하고, 쿠폰 고객은 제외한다.','카드 회수율과 달리 재방문율은 두 번째 방문 고객 수 ÷ 첫 방문 고객 수로 본다.'])assert.equal(status('revisit_cohort_definition',role(s+'\n'+long)),'fail',s)});
