...
check('registry has the thirteen v1 failure types and the copy pack grader',()=>assert.deepEqual([...GRADERS.map(g=>g.id)],['question_only','thin_section','contract_json','heading_nesting','internal_id_exposure','brief_prohibition_conflict','fact_conflict','unconfirmed_value_assertion','unsupported_claim_term','industry_metric_leak','revisit_cohort_definition','local_channel_coverage','input_budget','copy_pack_variants']));
