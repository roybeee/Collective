...
const originalOwner=owner;owner='second-owner';await snapshot();await act('save_hermes',{endpoint:'https://hermes.example.com',key:'hermes-local-test-key-only'});check('same client meeting id is isolated per owner',(await mp('start',args)).status===200);await mp('cancel',{id:args.id});owner=originalOwner;
