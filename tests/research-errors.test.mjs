...
// 파서가 null 실험 과제에서 TypeError를 내는 동작 자체는 tests/validate.test.mjs가 고정한다(A7에서 422로 바뀌면 이 스위트의 비-ApiError 유발 방법도 다른 내부 예외로 바꾼다). 이 스위트는 그 원문이 조사 기록·응답·로그로 새지 않는지 본다.
...
// 대화형 심층 조사(단계 1개): start → advance(제출) → advance(완료 조회·결과 처리).
async function run(id,output){
 nextOutput=output;
...
try{
 ...
 ({body,saved}=await run('re-type',report(x=>{x.diagnosis.opportunities=[null]})));
 ...
 check('internal exception text never reaches the record or the response',!/Cannot read|TypeError|reading '/.test(JSON.stringify(saved)+JSON.stringify(body)));
 ...
 check('logs carry only primitive values (no exception text or stack)',logged.every(a=>a.every(v=>v===null||['string','number'].includes(typeof v)))&&!/Cannot read|reading '|\n\s+at /.test(JSON.stringify(logged)));
