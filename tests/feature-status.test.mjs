...
check('row keys are unique and in display order',rows.map(r=>r.key),['brand','ai','text','review','quality-ops','metrics','worker','png','buffer','measurement','franchise','data-requests','pos-csv','pos-auto','video','ads']);
...
check('unknown live states say the state could not be checked',none.filter(r=>r.status==='blocked').map(r=>r.reason),['AI 연결 상태를 확인하지 못했습니다','작업자 상태를 확인하지 못했습니다','확정 사실 수를 확인하지 못했습니다','Buffer 연결 상태를 확인하지 못했습니다','채널 연결 상태를 확인하지 못했습니다','가맹 모집 스위치 상태를 확인하지 못했습니다','자료 요청 스위치 상태를 확인하지 못했습니다']);
