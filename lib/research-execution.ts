...
export async function executeResearch(owner:string,b:Record<string,any>,submissionTimeoutMs=90000){let lock='',lockOwner='',prepared:BrandResearch|undefined,current:BrandResearch|undefined,recovering=false;try{
 ...
 if(b.action==='start'){
  ...
  r={execution:serverMode?'server':'interactive',...(mode==='deep'?{protocol:DEEP_RESEARCH_VERSION,plan:defaultResearchPlan(brand),access:unverifiedResearchAccess()}:{}),...(previous?{previousResearchId:previous.id}:{}),id,brandId,mode,status:'running',steps:mode==='deep'?researchSteps(id,serverMode):[...Array.from({length:Math.ceil(sources.length/30)},(_,i)=>({id:id+'-step-'+i,stage:'identity' as const,status:'pending' as const,sourceIds:sources.slice(i*30,(i+1)*30).map(s=>s.id)})),{id:id+'-diagnosis',stage:'diagnosis',status:'pending'}],model:cfg.model,stopRequested:false,createdAt:stamp(),updatedAt:stamp(),tokens:0,snapshot:{brand,observations:(await listRecords<ChannelObservation>(owner,'brand_observation',brandId)).slice(0,12)}};
 ...
 const result=await pollHermes(cfg,step.providerId,r.stopRequested,Math.min(submissionTimeoutMs,45000));r.updatedAt=stamp();r.lastCheckedAt=r.updatedAt;r.activity=result.activity;r.activityAt=result.activityAt;r.retryAt=undefined;r.retryCount=0;r.error=result.needsApproval?'HERMES에서 도구 사용 승인을 기다리고 있습니다. 연결된 HERMES에서 요청 내용을 확인하세요.':undefined;r.status='running';step.status='running';
 ...
 if(result.status==='completed'){
  ...
  try{
   ...
   if(step.stage==='investigation'){
    ...
    const sources=parseResearchSources(x.sources,r.brandId,r.id,step.id,stamp()).filter(s=>!all.some(a=>a.url===s.url&&a.status!=='excluded'));if(r.mode==='classify'&&sources.length)throw new ApiError(422,'자료 분류에서 외부 출처를 만들 수 없습니다.');if(all.length+sources.length>200)throw new ApiError(400,'자료 보관 한도를 초과했습니다.');
 ...
 }else if(['failed','cancelled'].includes(result.status)){r.status=result.status as 'failed'|'cancelled';step.status='failed';r.error=result.failureReason||'HERMES 조사가 종료됐습니다. 저장된 자료에서 이어서 새 조사를 시작할 수 있습니다.';await database().batch(writes(owner,r))}
