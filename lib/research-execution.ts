...
import {unverifiedResearchAccess} from './research-queue';
...
export async function executeResearch(owner:string,b:Record<string,any>,submissionTimeoutMs=90000){let lock='',lockOwner='',prepared:BrandResearch|undefined,current:BrandResearch|undefined,recovering=false;try{
 ...
 if(b.action==='start'){
  ...
  const previous=b.previousResearchId?await readRecord<BrandResearch>(owner,'brand_research',str(b.previousResearchId,'이전 조사',70,true)):undefined;if(previous&&(previous.brandId!==brandId||researchActive(previous)))throw new ApiError(409,'종료된 같은 브랜드의 조사만 보완할 수 있습니다.');
  r={...(mode==='deep'?{protocol:DEEP_RESEARCH_VERSION,plan:defaultResearchPlan(brand),access:unverifiedResearchAccess()}:{}),...(previous?{previousResearchId:previous.id}:{}),id,brandId,mode,status:'running',steps:mode==='deep'?[{id:id+'-investigation',stage:'investigation',status:'pending'}]:[...Array.from({length:Math.ceil(sources.length/30)},(_,i)=>({id:id+'-step-'+i,stage:'identity' as const,status:'pending' as const,sourceIds:sources.slice(i*30,(i+1)*30).map(s=>s.id)})),{id:id+'-diagnosis',stage:'diagnosis',status:'pending'}],model:cfg.model,stopRequested:false,createdAt:stamp(),updatedAt:stamp(),tokens:0,snapshot:{brand,observations:(await listRecords<ChannelObservation>(owner,'brand_observation',brandId)).slice(0,12)}};
 ...
 if(step.status==='pending'){
  const archived=await listRecords<ArchiveSource>(owner,'brand_source',r.brandId),sources=archived.filter(s=>s.status!=='excluded');
  ...
  const input={protocol:r.protocol,plan:r.plan,access:r.access,maxNewSources:Math.max(0,Math.min(40,200-archived.length)),previousGaps:prior?.report?.quality.issues||[],brand:r.snapshot.brand,mode:r.mode,stage:step.stage,requestedAt:stamp(),sources:selected.map(s=>({id:s.id,title:s.title,status:s.status,category:s.category,url:s.url,scope:s.scope,observedAt:s.observedAt,content:s.content.slice(0,4500),excerpt:s.content.length>4500})),omittedSources:Math.max(0,sources.length-selected.length),observations:r.snapshot.observations.slice(0,12),priorSteps:r.steps.filter(s=>s.status==='completed').map(s=>({stage:s.stage,summary:s.summary,limitations:s.limitations}))};
  step.status='uncertain';r.updatedAt=stamp();await database().batch([...writes(owner,r),hermesSubmissionStatement(owner,step.id,{instructions:step.stage==='investigation'?deepInstructions:archiveResearchInstructions(step.stage,r.mode),input:JSON.stringify(input)},r.brandId)]);prepared=r;
 ...
 const result=await pollHermes(cfg,step.providerId,r.stopRequested,Math.min(submissionTimeoutMs,45000));r.updatedAt=stamp();r.retryAt=undefined;r.retryCount=0;r.error=result.needsApproval?'HERMES에서 도구 사용 승인을 기다리고 있습니다. 연결된 HERMES에서 요청 내용을 확인하세요.':undefined;r.status='running';step.status='running';
 ...
 if(result.status==='completed'){
  ...
  try{
   ...
   step.status='completed';r.tokens+=Number.isFinite(result.usage.total_tokens)?Math.max(0,result.usage.total_tokens):0;if(r.steps.every(s=>s.status==='completed'))r.status='completed';
...
}catch(e){if(prepared){prepared.status=recovering||!(e instanceof ApiError)||e.status>=500||e.status===429?'uncertain':'failed';prepared.error=prepared.status==='uncertain'?'조사 접수 확인이 지연되고 있습니다. 같은 요청으로 다시 확인하며 중복 조사를 만들지 않습니다.':(e as Error).message;prepared.updatedAt=stamp();if(!(e instanceof ApiError)||e.status>=500||e.status===429)researchRetry(prepared);else prepared.retryAt=undefined;await database().batch(writes(owner,prepared));return json(publicResearch(prepared))}if(current&&e instanceof ApiError&&(e.status>=500||e.status===429)){current.error='연결 응답이 늦어 다시 확인합니다. 접수된 HERMES 조사는 계속 진행됩니다.';current.updatedAt=stamp();researchRetry(current);await database().batch(writes(owner,current));return json(publicResearch(current))}return failure(e)}finally{if(lock)await releaseLock(lockOwner,lock)}}
