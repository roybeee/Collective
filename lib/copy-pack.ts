...
export const copyPackSchema='copyPack:{version:"copy-pack-v2",shortform:{channel:"숏폼 채널",durationSec:15,scenes:[{start:0,end:3,visual:"화면",line:"대사",caption:"자막",sound:"소리",transition:"전환"}]},experiments:[{title:"실험 이름",channel:"channels의 채널 이름",hypothesis:"가설",variable:"바꿀 요소 하나",control:"대조 안 id",treatment:"실험 안 id",fixed:"고정할 요소",metric:"share_rate|completion_rate|click_rate"}],channels:[{channel:"채널 이름",purpose:"용도",destination:"연결 목적지",variants:[{id:"A",angle:"설득 각도",hook:"훅",body:"본문",cta:"CTA 하나",needsCheck:["확인할 항목"]}]}]}';
export const copyPackInstruction=` 카피 팩 규칙: channels는 1~4개이고 브리프 채널을 우선합니다. 채널마다 variants를 3~5안 쓰고 안마다 훅과 본문이 서로 달라야 합니다(띄어쓰기·문장부호만 다른 안은 같은 안으로 셉니다). 안 id는 채널 안에서 겹치지 않게 A·B·C 순서로 씁니다. hook은 ${COPY_PACK_LIMITS.hook}자, body는 ${COPY_PACK_LIMITS.body.toLocaleString('en-US')}자, cta는 ${COPY_PACK_LIMITS.cta}자 이하이고 CTA는 하나만 씁니다. 검증되지 않은 가격·효능은 문안에 [확인 필요]로 두고 needsCheck에 항목 이름을 적습니다. shortform.scenes는 3~10개이며 0초에서 시작해 빈틈 없이 이어지고 마지막 end가 durationSec(6~60초)와 같아야 합니다. experiments는 1~3개이고 control·treatment는 같은 channel의 서로 다른 안 id, metric은 share_rate·completion_rate·click_rate 중 하나입니다. copyPack은 shortform, experiments, channels 순서로 쓰고, channels 배열에는 채널 객체만 넣습니다(실험은 experiments에만). 응답 JSON은 열린 괄호를 모두 닫아 끝냅니다. 특히 copyPack 객체와 최상위 객체를 닫는 }를 빠뜨리지 말고, 마지막 문자가 최상위 객체를 닫는 }인지 확인하세요. 시스템이 팩을 output_1(채널별 카피 안)·output_2(장면표)·output_4(실험) 앞에 붙이므로 sections에는 팩 문안을 반복하지 말고 채널별 용도·선택 이유, 편집 메모, 랜딩 문안, 실험 선택 이유를 짧게 쓰세요.`;
