...
export type LearningRule={origin?:'viral'|'store';storeId?:string;storeAssessment?:StoreAssessment;renewCount?:number;renewedAt?:string;renewReason?:string;direction?:'test'|'caution';sourceAssessment?:{status:'promising'|'not_supported';metric:LearningMetric;controlRate:number|null;treatmentRate:number|null;lift:number|null;controlSample:number|null;treatmentSample:number|null;startedAt:string|null;observedUntil:string;conditions:string;notes:string;stats?:ViralStats|null;decision?:'adopt'|'stop';decisionConflict?:'interim'|'mismatch';decisionReason?:string};id:string;brandId:string;channel:string;experimentId:string;experimentVersion:number;caseId:string;title:string;guidance:string;scope:string;evidenceLevel:'observational';status:'active'|'paused'|'retired';version:number;expiresAt:string;createdAt:string;updatedAt:string};
