...
// 소비자 30일 재방문율 정의(measurementDiscipline)는 objective 없는 캠페인 바이트를 지키려고 지시문에 그대로 두고, 여기서 이 캠페인에 적용하지 말라고 적는다.
