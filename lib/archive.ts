...
export type Diagnostic={id:string;brandId:string;researchId:string;archiveRevision:number;status:'candidate'|'confirmed';summary:string;positioning:string;audience:string;needs:string;strengths:string;gaps:string;opportunities:{title:string;hypothesis:string;action:string;metric:string;sourceIds:string[]}[];questions:string[];sourceIds:string[];limitations:string;createdAt:string};
export type ResearchStage='identity'|'customer'|'channel'|'diagnosis';
export const researchStages:Record<ResearchStage,string>={identity:'브랜드·제품 이해',customer:'고객·시장·경쟁 조사',channel:'채널·콘텐츠 진단',diagnosis:'종합 진단·전략 과제'};
export type BrandResearch={id:string;brandId:string;mode:'deep'|'classify';status:'running'|'uncertain'|'completed'|'failed'|'cancelled';steps:{id:string;stage:ResearchStage;status:'pending'|'running'|'uncertain'|'completed'|'failed';sourceIds?:string[];providerId?:string;summary?:string;limitations?:string}[];model:string;stopRequested:boolean;error?:string;createdAt:string;updatedAt:string;snapshot:{brand:unknown;observations:ChannelObservation[]};tokens:number};
