...
export type BrandResearch={lastCheckedAt?:string;activity?:string;activityAt?:string;retryAt?:string;retryCount?:number;protocol?:string;plan?:import('./deep-research').ResearchPlan;access?:import('./deep-research').ResearchAccess;report?:import('./deep-research').DeepReport;previousResearchId?:string;id:string;brandId:string;mode:'deep'|'classify';status:'running'|'uncertain'|'completed'|'failed'|'cancelled';steps:{id:string;stage:ResearchStage;status:'pending'|'running'|'uncertain'|'completed'|'failed';sourceIds?:string[];providerId?:string;rawResult?:string;summary?:string;limitations?:string}[];model:string;stopRequested:boolean;error?:string;createdAt:string;updatedAt:string;snapshot:{brand:unknown;observations:ChannelObservation[]};tokens:number};
