...
// A7 부분 구제 기록. 심층 조사 보고서에서 뺀 원소마다 위치·번호·사유를 모두, 목록별 남은 개수와 함께 조사 기록에 남긴다. 뺀 원소의 원문은 저장하지 않는다.
export type SalvageDrop={section:string;index:number;id?:string;reason:string};
export type ResearchSalvage={dropped:SalvageDrop[];kept:Record<string,number>};
export const salvageSections:Record<string,string>={sources:'출처',access:'접근 기록',cases:'콘텐츠 표본',customerSignals:'고객 관찰',competitors:'경쟁·대안','review.claims':'핵심 주장 검토','review.followups':'보완 조사','review.unresolved':'미해결 질문','diagnosis.sourceIds':'진단 근거','diagnosis.opportunities':'실험 과제','diagnosis.questions':'확인 질문'};
export type ResearchStage='store_diagnosis'|'investigation'|'identity'|'customer'|'channel'|'diagnosis';
export const researchStages:Record<ResearchStage,string>={store_diagnosis:'점포 진단·실험 설계',investigation:'심층 온보딩 조사',identity:'브랜드·제품 이해',customer:'고객·시장·경쟁 조사',channel:'채널·콘텐츠 진단',diagnosis:'종합 진단·전략 과제'};
export type BrandResearch={storeId?:string;execution?:'server'|'interactive';lastCheckedAt?:string;activity?:string;activityAt?:string;retryAt?:string;retryCount?:number;protocol?:string;plan?:import('./deep-research').ResearchPlan;access?:import('./deep-research').ResearchAccess;report?:import('./deep-research').DeepReport;salvage?:ResearchSalvage;previousResearchId?:string;id:string;brandId:string;mode:'deep'|'classify';status:'running'|'uncertain'|'completed'|'failed'|'cancelled';steps:{id:string;stage:ResearchStage;status:'pending'|'running'|'uncertain'|'completed'|'failed';sourceIds?:string[];providerId?:string;rawResult?:string;summary?:string;limitations?:string}[];model:string;stopRequested:boolean;error?:string;createdAt:string;updatedAt:string;snapshot:{brand:unknown;store?:import('./store-marketing').Store;storeContext?:unknown;observations:ChannelObservation[]};tokens:number};
