...
export type NavState={view:NavView;campaign?:string;brand?:string;store?:string;tab?:LearningTab};
...
const isTab=(value:string|null|undefined):value is LearningTab=>(learningTabs as readonly string[]).includes(value as string);
// brand는 브랜드 아카이브·점포 마케팅·학습에서만, store는 점포 마케팅에서만, tab은 학습에서만 의미가 있다.
export function normalizeNav(input:NavInput):NavState{
 ...
 const brand=view==='brands'||view==='stores'||view==='learning'?validId(input.brand):undefined,store=view==='stores'?validId(input.store):undefined,tab=view==='learning'&&isTab(input.tab)?input.tab:undefined;
...
// 불러온 데이터에 없는 id: 캠페인은 캠페인 목록으로, 브랜드는 같은 화면의 전체 목록으로(학습 탭은 유지). 바뀌지 않으면 같은 객체를 돌려준다.
export function reconcileNav(state:NavState,known:{campaigns:string[];brands:string[]}):NavState{
 ...
 if(state.brand&&!known.brands.includes(state.brand))return {view:state.view,...(state.campaign?{campaign:state.campaign}:{}),...(state.tab?{tab:state.tab}:{})};
 return state;
}
