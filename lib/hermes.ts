...
export async function hermesRequest(cfg:Connection,path:string,init:RequestInit={}){
 let r:Response;try{r=await fetch(hermesEndpoint(cfg.endpoint!)+path,{...init,redirect:'error',headers:{...init.headers,Authorization:`Bearer ${cfg.key}`,'Content-Type':'application/json'},signal:AbortSignal.timeout(12000)})}catch{throw new ApiError(502,'HERMES에 연결하지 못했습니다. gateway와 연결 주소를 확인하세요. OpenAI API로 전환하지 않았습니다.')}
...
export async function verifyHermes(cfg:Connection){
 ...
 let unauth:Response;try{unauth=await fetch(cfg.endpoint+'/v1/capabilities',{redirect:'error',signal:AbortSignal.timeout(12000)})}catch{throw new ApiError(502,'HERMES 인증 보호를 확인하지 못했습니다.')}
