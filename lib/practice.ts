...
export const channelSkills:readonly {id:string;applies:(c:ChannelScope)=>boolean;body:string}[]=[
 ...
 {id:'expo',applies:recruiting(/박람회|설명회|엑스포|expo|견학/i),body:'박람회·설명회: 부스·설명회 QR에 유입 코드를 붙이고 현장 기록 항목(방문 시각, 관심 지역, 희망 시기, 연락 동의)을 정한다. 행사 뒤 48시간 안에 사람이 연락하는 순서를 둔다. 설명회 자료는 승인된 버전만 쓰고 수익에 관한 질문은 정보공개서와 서면 절차 안내로 답한다. 부스 배너·리플렛도 표시·광고 대상이다. 브리프나 상시 지시에 참가 제한이 있으면 참가 대신 관찰 계획만 쓴다.'},
 ...
 {id:'referral',applies:recruiting(/추천|(?<!예비\s?)점주|qr|큐알/i),body:'점주·지인 추천: 매장 QR은 소비자 동선(주문·방문)과 모집 동선을 서로 다른 유입 코드로 나눈다. 추천 사례는 실재 점포와 본인 동의가 있는 것만 쓰고 추천인의 경제적 이해관계를 표시한다. 추천 보상은 제안하지 않는다. 추천인이 가맹 중개에 해당하는지 확인되기 전에는 추천인이 소개만 하고 상담 안내는 본부가 맡는다.'},
];
...
export function channelSkillIds(c:ChannelScope){const ids=appliedChannelSkills(c).map(s=>s.id);return ids.length?ids:['default']}
export function campaignPractice(c:ChannelScope,channels?:Record<string,string>){
 const guides=appliedChannelSkills(c).map(s=>channels?.[s.id]??s.body);
