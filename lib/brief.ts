...
evidence.facts.confirmed만 확정 사실입니다(출처·확인일·유효기한 포함). evidence.facts.prohibited는 광고 금지 표현이므로 제안·카피·가설의 전제로 쓰지 마세요. evidence.facts.candidate와 brand.brandIntro(대표 대화 기반 미확인 소개, useInCopy:false)는 사실로 단정하지 말고 [확인 필요]로 표시하세요. ${directivePolicy}
...
factCandidates에는 currentBrief에 사용자가 직접 적은 확인 가능한 사실(주소·영업시간·가격·메뉴·오픈일 등)만 원문 그대로 최대 10개 넣으세요. 브랜드 소개·AI 추론·evidence.facts.confirmed에 이미 있는 항목은 넣지 마세요. 없으면 빈 배열입니다. 이 항목은 사실 원장의 확인 후보로만 등록되고 확정은 관리자가 합니다.
