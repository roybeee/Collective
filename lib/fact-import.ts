...
export function guessFactKey(value:string):string|undefined{return keyPatterns.find(([,re])=>re.test(value))?.[0]}
...
export function ledgerChecks(brand:Brand,stores:Store[],facts:BrandFact[]):LedgerCheck[]{
 ...
 for(const field of ['description','knowledge'] as const)String(brand[field]||'').split(/(?<=[.!?。])\s+|\n+/).map(s=>s.trim()).filter(Boolean).forEach((sentence,i)=>{
  for(const [key,re] of coreClaims){const terms=[...new Set(sentence.match(re)||[])];const verdict=terms.length?judge(brandFacts,brandConfirmed,key,terms,false):null;if(verdict)checks.push({id:`brand:${field}:${i}:${key}`,origin:'brand',field,key,label:factLabel(key),claim:sentence.slice(0,1000),value:terms.join(', '),source:`브랜드 소개 (brand.${field})`,...verdict})}
 });
 for(const s of stores){
  ...
  for(const [key,re] of coreClaims.slice(0,2)){const terms=[...new Set(menu.match(re)||[])];const verdict=terms.length?judge(scoped,confirmed,key,terms,false):null;if(verdict)checks.push({...base,id:`store:${s.id}:menu:${key}`,field:'menu',key,label:factLabel(key),claim:menu.slice(0,1000),value:terms.join(', '),source:base.source+' · store.menu',...verdict})}
 }
 return checks;
}
