...
export function channelInput(raw:any,storeId:string,old?:StoreChannel):StoreChannel{
 const key=option(raw.key,channelCatalog.map(c=>c.key),'채널'),catalog=channelCatalog.find(c=>c.key===key)!;
 const checks=Object.fromEntries(catalog.checks.map((label,i)=>[String(i),option(raw.checks?.[i]??'unknown',Object.keys(checkStates) as (keyof typeof checkStates)[],label)]));
...
export function experimentInput(raw:any){
...
export function measurementInput(raw:any,experiment:StoreExperiment,old?:StoreMeasurement):StoreMeasurement{
...
export function parseStoreReport(raw:any,store:Store,sources:ArchiveSource[],id:string):StoreReport{
 ...
 const arr=(v:any,max:number)=>{if(!Array.isArray(v)||v.length>max)throw new ApiError(422,'점포 진단 목록 형식을 확인하세요.');return v};
 const refs=(v:any)=>{const ids=arr(v,40);if(ids.some(x=>typeof x!=='string'||!allowed.has(x)))throw new ApiError(422,'점포 진단에 실제 자료와 일치하지 않는 근거가 있습니다.');return [...new Set(ids)] as string[]};
 const sourceIds=refs(raw.sourceIds),itemRefs=(v:any)=>{const ids=refs(v);if(!ids.length||ids.some(x=>!sourceIds.includes(x)))throw new ApiError(422,'실행 제안에 전체 진단과 연결된 근거가 필요합니다.');return ids};
 const text=(v:any,label:string,max=2000)=>str(v,label,max,true);
 return {id,storeId:store.id,brandId:store.brandId,storeVersion:store.version,status:'candidate',summary:text(raw.summary,'진단 요약'),customer:text(raw.customer,'우선 고객'),bottleneck:text(raw.bottleneck,'방문 장벽'),measurementPlan:text(raw.measurementPlan,'측정 설계'),limitations:text(raw.limitations,'조사 한계'),questions:arr(raw.questions,8).map(q=>text(q,'질문')),sourceIds,actions:arr(raw.actions,8).map(a=>({channel:option(a.channel,channelCatalog.map(c=>c.key),'채널'),priority:option(a.priority,['first','next'] as const,'우선순위'),action:text(a.action,'수정 과제'),reason:text(a.reason,'이유'),sourceIds:itemRefs(a.sourceIds)})),proposals:arr(raw.proposals,2).map(p=>({title:text(p.title,'실험 이름',150),channel:option(p.channel,channelCatalog.map(c=>c.key),'채널'),hypothesis:text(p.hypothesis,'가설'),control:text(p.control,'비교 조건'),treatment:text(p.treatment,'바꿀 변수'),measurement:text(p.measurement,'측정 방법'),sourceIds:itemRefs(p.sourceIds)})),createdAt:stamp()};
}
