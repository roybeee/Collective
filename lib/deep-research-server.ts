...
2. 사용 가능한 도구에서 Aside를 발견하면 공식 SNS 주소를 실제 브라우징하여 조사하세요. Aside가 없으면 사용 가능한 읽기 전용 브라우저·검색·공식 API·업로드를 쓰고 어떤 도구로 어디까지 확인했는지 남기세요. 검색 스니펫만 본 것은 원문 읽음이나 영상 시청이 아닙니다. API에 비공개 권한이 없으면 도달·저장·전환·매출을 추정하지 마세요.
