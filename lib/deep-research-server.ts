...
export async function inspectResearchAccess(cfg:Connection):Promise<ResearchAccess>{
 ...
 if(caps.endpoints?.toolsets?.method==='GET'&&caps.endpoints.toolsets.path==='/v1/toolsets'){
  try{const list=await hermesRequest(cfg,'/v1/toolsets');if(!Array.isArray(list.data))throw 0;const enabled=list.data.filter((x:any)=>x.enabled===true&&x.configured===true);result.tools=[...new Set<string>(enabled.flatMap((x:any)=>[x.name,...(Array.isArray(x.tools)?x.tools:[])].filter((t:any)=>typeof t==='string'&&/^[\w:.-]{1,120}$/.test(t))))].slice(0,100);if(result.tools.some(t=>/aside/i.test(t)))result.aside='advertised';if(result.tools.some(t=>/browser|aside|playwright/i.test(t)))result.browser='advertised'}catch{result.notes.push('도구 목록을 읽지 못했습니다. 실제 실행에서 접근 가능 여부를 확인합니다.')}
...
const arr=(v:any,label:string,max:number)=>{if(!Array.isArray(v)||v.length>max)throw new ApiError(422,label+' 형식을 확인하세요.');return v};
const text=(v:any,label='조사 내용',max=1000)=>str(v,label,max,true);
const choice=<T extends string>(v:any,allowed:readonly T[],label:string):T=>{if(!allowed.includes(v))throw new ApiError(422,label+' 값이 올바르지 않습니다.');return v};
const number=(v:any,label:string,nullable=true)=>{if(v===null&&nullable)return null;if(typeof v!=='number'||!Number.isFinite(v)||v<0||v>1e12)throw new ApiError(422,label+' 수치를 확인하세요.');return v as number};
export function parseDeepReport(x:any,r:BrandResearch,existing:ArchiveSource[]){
 ...
 const sources=arr(x.sources,'조사 출처',40).map((s:any,i:number)=>{const id=text(s?.id,'자료 번호',80);if(!/^[a-zA-Z0-9_-]+$/.test(id)||refs.has(id))throw new ApiError(422,'중복되거나 잘못된 출처 번호입니다.');const url=archiveUrl(s.url,true);if(urls.has(url))throw new ApiError(422,'같은 출처 URL이 중복됐습니다.');urls.add(url);text(s.content,'자료 요약',2000);text(s.scope,'확인 범위',1000);const parsed=parseResearchSources([s],r.brandId,r.id,r.id+'-e'+i,stamp())[0];refs.set(id,parsed.id);return parsed;});
 const reference=(id:any)=>{if(typeof id!=='string'||!refs.has(id))throw new ApiError(422,'실제 조사 자료와 일치하지 않는 근거입니다.');return refs.get(id)!};
 const references=(ids:any)=>[...new Set<string>(arr(ids,'근거 목록',40).map(reference))];
 const phases=arr(x.phases,'조사 단계',6).map((p:any,i:number)=>{if(p?.phase!==researchPhases[i])throw new ApiError(422,'조사 단계 기록이 누락되거나 순서가 다릅니다.');return {phase:p.phase,summary:text(p.summary,'단계 요약',1500)}});if(phases.length!==6)throw new ApiError(422,'여섯 조사 단계의 기록이 필요합니다.');
 const access=arr(x.access,'접근 기록',70).map((a:any)=>({sourceId:reference(a?.sourceId),method:choice(a.method,['browser','api','upload','search_snippet'] as const,'접근 방식'),tool:text(a.tool,'사용 도구',200),scope:text(a.scope,'접근 범위')}));
 ...
 const cases:ContentCase[]=arr(x.cases,'콘텐츠 표본',30).map((c:any)=>{const id=text(c?.id,'콘텐츠 번호',80),sourceId=reference(c.sourceId);if(ids.has(id)||caseSources.has(sourceId))throw new ApiError(422,'콘텐츠 표본이 중복됐습니다.');ids.add(id);caseSources.add(sourceId);
  ...
  const viewedRanges=arr(c.viewedRanges,'시청 구간',10).map((v:any)=>{const start=number(v?.start,'시작 초',false)!,end=number(v?.end,'종료 초',false)!;if(end<=start||durationSeconds===null||end>durationSeconds)throw new ApiError(422,'시청 구간을 확인하세요.');return {start,end}}).sort((a,b)=>a.start-b.start);
  ...
  const timeline=arr(c.timeline,'장면 분석',8).map((t:any)=>{const second=number(t?.second,'장면 시점',false)!;if(!viewedRanges.some(v=>v.start<=second&&second<=v.end))throw new ApiError(422,'보지 않은 영상 구간을 분석할 수 없습니다.');return {second,observation:text(t.observation,'장면 관찰',500)}});
 ...
 const customerSignals=arr(x.customerSignals,'고객 관찰',20).map((s:any)=>({sourceId:reference(s?.sourceId),kind:choice(s.kind,['motivation','barrier','complaint','question'] as const,'고객 관찰 유형'),observation:text(s.observation),implication:text(s.implication)}));
 const competitors=arr(x.competitors,'경쟁·대안',5).map((c:any)=>({name:text(c?.name,'경쟁사',100),sourceIds:references(c.sourceIds),difference:text(c.difference)}));if(competitors.some(c=>!c.sourceIds.length)||new Set(competitors.map(c=>c.name.trim().toLowerCase())).size!==competitors.length)throw new ApiError(422,'경쟁사 근거가 없거나 중복됐습니다.');
 const review={claims:arr(x.review?.claims,'핵심 주장 검토',8).map((c:any)=>({claim:text(c?.claim),sourceIds:references(c.sourceIds),counterEvidence:text(c.counterEvidence),nextCheck:text(c.nextCheck)})),followups:arr(x.review?.followups,'보완 조사',r.plan.maxFollowups).map((f:any)=>({question:text(f?.question),finding:text(f.finding),sourceIds:references(f.sourceIds)})),unresolved:arr(x.review?.unresolved,'미해결 질문',12).map((v:any)=>text(v))};
 const all=[...existing,...sources];const diagnosis=parseDiagnostic({...x.diagnosis,sourceIds:references(x.diagnosis?.sourceIds),opportunities:arr(x.diagnosis?.opportunities,'실험 과제',3).map((o:any)=>({...o,sourceIds:references(o.sourceIds)}))},all);
