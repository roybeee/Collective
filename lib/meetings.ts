...
export type QualityReview={verdict:'ready_for_review'|'revise'|'needs_data';summary:string;findings:string};
...
export function parseMeetingOutput(text:string,step:MeetingStep,previous:MeetingStep[]){
 ...
 return {verdict:x.verdict,summary:required(x,'summary'),findings:required(x,'findings')} as QualityReview;
}
...
export function meetingInstructions(step:MeetingStep){
 ...
 if(step.phase==='synthesis')return base+'모든 팀원의 발언을 검토하고 실행 가능한 합의안을 만드세요. 의견이 다르면 해결 근거 또는 남은 쟁점을 명시하며 억지로 합의시키지 마세요. 서로 의존하는 순서로 1~3개 담당자의 개선 과제를 지정하세요. 상위 전략을 바꾸면 이후 미수정 작업물이 이전 버전으로 바뀐다는 점을 고려하세요. 실적이 없어도 실행 가능한 카피/실험 설계 등을 개선할 수 있습니다. 형식: {"decisions":"채택할 방향과 근거","disagreements":"기각/보류 의견과 이유 또는 없음","questions":"사용자에게 필요한 사실 확인 또는 없음","tasks":[{"role":"cmo|insight|strategy|creative|content|growth|data","instruction":"완성할 작업물과 수정 사항","reason":"회의 근거","acceptance":"품질 담당자가 확인할 구체적 완료 조건"}]}. 같은 role은 중복 금지. quality에는 과제를 배정하지 마세요.';
 if(step.phase==='revision')return base+'배정된 task와 앞선 개선본을 반영한 완성된 작업물을 작성하세요. 수정 계획만 쓰지 마세요. 이전 작업물을 그대로 반복하지 마세요. 형식: {"title":"작업물 제목","content":"완성된 카피/대본/전략/실험 설계 본문. 마크다운 가능","changes":"어느 팀원의 지적과 합의 과제를 어떻게 반영했는지"}. 실제 제작 파일/게시를 완료했다고 말하지 마세요.';
 return base+'당신은 수정본을 작성한 담당자와 별도로 품질을 재검토합니다. candidateArtifacts만 현행 후보 작업물입니다. tasks의 acceptance를 하나씩 대조하고 누락·충돌·근거 부족을 특정하세요. invalidatedRoles가 있으면 후속 작업 필요를 명시하세요. 형식: {"verdict":"ready_for_review|revise|needs_data","summary":"검수 결론","findings":"근거·브랜드·제작 완성도·측정 가능성별 발견 위치, 문제와 다음 담당자 수정 요청"}. ready_for_review는 사용자 검토 준비이지 승인이나 성과 보장이 아닙니다. 중요한 사실이 없으면 needs_data, 수정할 내용이 있으면 revise.';
}
