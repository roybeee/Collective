...
export async function learningAction(owner:string,b:any){
 ...
 if(['retire_rule','renew_rule','retest_rule'].includes(b.action)){
  ...
  await database().batch([recordStatement(owner,'viral_experiment',id,retest,campaign.id),eventStatement(owner,campaign.id,`「${r.title}」 규칙의 재검증 실험을 만들었습니다. 만료 전에 다시 측정하세요.`)]);
  return {id};
 }
