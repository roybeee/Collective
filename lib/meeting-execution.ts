...
export async function executeMeeting(owner:string,b:Record<string,unknown>){let lock='',prepared:Meeting|undefined,recovering=false;
 ...
 lock=await acquireLock(owner);
  ...
  if(!['advance','recover','cancel'].includes(String(b.action)))throw new ApiError(400,'지원하지 않는 회의 작업입니다.');
  ...
  if(result.status==='completed'){
   ...
   catch(e){await markUsageOutcome(owner,'hermes',s.providerId,'invalid_output');s.status='failed';s.error=(e as Error).message;m.status='failed';m.error=s.error;await database().batch(writes(owner,m));return json(publicMeeting(m))}
   ...
   await markUsageOutcome(owner,'hermes',s.providerId,result.invalidOutput?'invalid_output':result.status==='cancelled'?'cancelled':'provider_failed');s.status=result.status;m.status=result.status;m.error='담당자 작업이 종료되어 회의를 멈췄습니다. 기록을 바탕으로 후속 회의를 시작할 수 있습니다.';await database().batch(writes(owner,m));
  }else await database().batch(writes(owner,m));
  return json(publicMeeting(m));
 }catch(e){
  if(prepared){
