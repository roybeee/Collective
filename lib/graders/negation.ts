...
// 닫는 따옴표 뒤 인용 조사 '고'도 같다('‘체험 가격이 2만 원이다’, ‘가장 인기 있다’고 광고하지 않는다', 2026-09-25 fa9da73 로컬 채널 재실행 S8 총괄).
...
// '적용하'('… caution 규칙에 따라 적용하지 않는다', R3 기준선 S3 실측)도 표현 사용 동사다.
const USE_VERB='(?:쓰|사용하|적용하|넣|적|싣|담|표기하|표시하|노출하|언급하|주장하|내세우|강조하|활용하|제안하|권하|만들|요청하|작성하|게시하|올리|유도하)';
...
// '사용할 수 없습니다·쓸 수 없다'도 사용 부정이다(R3 기준선 S6 품질 검수 실측). '쓸 수 있다'는 아니다.
const CANNOT_USE='(?:쓸|사용할|넣을|적을|표기할|표시할|노출할|언급할|주장할|내세울|강조할|활용할|적용할)\\s?수\\s?(?:는\\s?|도\\s?)?없(?:다|습니다|음|고|으며|어요)?';
const FINAL_NEGATION=new RegExp(`(?:${USE_NEG}|(?:금지|삭제|제외|배제|보류|금물)${HOLD_END}|${AVOID}|${ABSENT_EXPR}|${DEFER}|${CANNOT_USE})$`);
// 대상 뒤 40자 밖의 먼 부정은 표현 사용의 부정·금지·사용 보류만 인정한다. 단독 '보류·삭제·제외'는 먼 앞 행위를 부정하지 않는다
// ('가짜 후기 3건을 올려 초기 평점을 만드는 방안, 외부 이벤트는 보류').
const FAR_NEGATION=new RegExp(`(?:${USE_NEG}|(?:금지|금물)${HOLD_END}|(?:사용|표기|노출|언급|게재|기재)\\s?(?:금지|삭제|제외|배제|보류)${HOLD_END}|${AVOID}|${ABSENT_EXPR}|${DEFER}|${CANNOT_USE})$`);
...
// '사용 여부'('AI 생성·합성 소재 사용 여부', 확인 항목)도 '여부'다(2026-09-25 fa9da73 로컬 채널 재실행 S2 총괄).
...
const RULE_NAME=/(?:주의|금지|경고|보류|피하기|자제)\s*$/,WHETHER=/^[’”"」』]?\s?(?:(?:제공\s?|적용\s?|사용\s?)?여부|인가요|인지|일까요|입니까)/;
// 대상을 쓰려는 조건('… ‘당일 전량 소진’ 표현을 사용하려는 경우', 수정·중단 조건)은 사용이 아니다(2026-09-25 fa9da73 로컬 채널 재실행 S2 총괄).
// '비가 오는 경우 ‘…’ 문구를 넣는다'처럼 조건이 대상 앞에 있으면 아니다.
const INTENT_CONDITION=/^[’”"」』]?\s?(?:표현|문구|카피|문안|단어|주장)?(?:을|를)?\s?(?:(?:사용|활용|표기|노출|언급|주장|기재|삽입|강조)하|쓰|넣으|적으|내세우|붙이)려(?:는|고\s?하는)\s?(?:경우|때)/;
...
export function negatedAt(s:string,start:number,end:number){
 ...
 if(inRuleName(s,a,start)||WHETHER.test(s.slice(end,end+8))||INTENT_CONDITION.test(s.slice(end,end+24))||TOPIC_NOT_FACT.test(s.slice(end,end+100))||GUARANTEE_QUESTION.test(s.trim()))return true;
...
// 전략의 '포기하는 것' 칸도 쓰지 않을 약속 목록이다(2026-09-25 fa9da73 로컬 채널 재실행 S6 전략).
const PROHIBITIVE=/(?:금지|보류|삭제|제외|배제|피할|포기하는|(?:사용|쓰|넣|하)지\s?않(?:을|는)?)\s?(?:표현|문구|카피|문안|단어|어휘|어|키워드|항목|목록|사항|대상|것|행위)?$/;
