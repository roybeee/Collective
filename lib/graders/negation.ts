...
const FINAL_NEGATION=new RegExp(`(?:${USE_NEG}|(?:금지|삭제|제외|배제|보류|금물)${HOLD_END}|${AVOID})$`);
// 대상 뒤 40자 밖의 먼 부정은 표현 사용의 부정·금지·사용 보류만 인정한다. 단독 '보류·삭제·제외'는 먼 앞 행위를 부정하지 않는다
// ('가짜 후기 3건을 올려 초기 평점을 만드는 방안, 외부 이벤트는 보류').
const FAR_NEGATION=new RegExp(`(?:${USE_NEG}|(?:금지|금물)${HOLD_END}|(?:사용|표기|노출|언급|게재|기재)\\s?(?:금지|삭제|제외|배제|보류)${HOLD_END}|${AVOID})$`);
