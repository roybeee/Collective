...
// 법정 기준을 옮긴 수('가맹점 수가 100개 이상인 가맹본부는 …', '가맹점 100개 이상이면')는 매장 수 주장이 아니다(R2 4차 재검토).
const LEGAL_COUNT='(?!\\s?(?:이상|이하|미만|초과)?\\s?(?:인\\s?(?:가맹\\s?)?(?:본부|본사|브랜드|경우|사업자|업체)|이면|일\\s?(?:경우|때)|(?:가맹\\s?)?본부))';
const CNT='\\d[\\d,]*(?:\\s?여)?',UNIT=`${CNT}\\s?(?:개|곳)(?:\\s?(?:이상|넘게|넘는|넘은|초과|이\\s?넘|을\\s?넘|이나))?${LEGAL_COUNT}`;
// 단위 없는 수는 '수' 라벨 뒤에서만 매장 수다('가맹점 수 벌써 120'). 순위·배수·비율·기간·연도·날짜('가맹점 수 2025-12-31 기준')는 아니다.
const BARE_CNT=`(?!(?:19|20)\\d{2}(?!\\d))\\d[\\d,]*(?![\\d%]|[,.\\-/]\\d|\\s?(?:개|곳|위|등|배|년|개월|일|명|호점|퍼센트|%|[A-Za-z]))${LEGAL_COUNT}`;
const COUNT_ADV='(?:(?:벌써|현재|이미|무려|어느덧|드디어|총|약)\\s?)';
...
export const FR_STORE_COUNT=[
 `(?:전국|국내|누적|어느덧|무려|드디어)(?:에서|에|의)?\\s?(?:(?:곳곳|방방곡곡|각지)(?:에서|에|의)?\\s?)?(?:${UNIT}|${CNT}(?=\\s?${STORE_NOUN}))\\s?(?:의\\s?)?${STORE_NOUN}${AVAIL}`,
 `(?:총|현재|벌써|이미)(?:에서|에|의)?\\s?${UNIT}\\s?(?:의\\s?)?${STORE_NOUN}${SUBSET_AFTER}`,
 `(?:전국|국내|총|누적|현재)\\s?(?:의\\s?)?(?:운영\\s?(?:중인\\s?)?)?${STORE_NOUN}\\s?(?:수\\s?)?(?:[은는이가]\\s?|[:：]\\s?)?${UNIT}${SUBSET_AFTER}`,
 `${SUBSET_BEFORE}${UNIT}\\s?(?:의\\s?)?${STORE_NOUN}(?:이|가|에서|을|를)?\\s?${RUNNING}`,
 `${SUBSET_BEFORE}${NOT_RATIO}(?:가맹\\s?(?:매장|점포)|직영\\s?(?:매장|점포)|가맹점|직영점)(?:\\s?[과와·]\\s?(?:직영점|가맹점))?\\s?(?:수\\s?|합계\\s?|총\\s?|전체\\s?)?(?:[은는이가]\\s?|[:：]\\s?)?${COUNT_ADV}?${UNIT}${SUBSET_AFTER}${NOT_OBJECT}`,
 `${SUBSET_BEFORE}${NOT_RATIO}(?:가맹\\s?(?:매장|점포)|직영\\s?(?:매장|점포)|가맹점|직영점|매장|점포|지점)\\s?수\\s?(?:[은는이가]\\s?|[:：]\\s?)?${COUNT_ADV}?${BARE_CNT}`,
 ...
 `${CNT}\\s?호점\\s?(?:돌파|달성|시대|눈앞|넘|(?:오픈\\s?)?임박|계약\\s?(?:완료|체결|달성|돌파))|(?:벌써|어느덧|무려|드디어|전국|누적)\\s?${CNT}\\s?호점|\\d+\\s?(?:년|개월|달)\\s?(?:만에|새|사이|동안)\\s?${CNT}\\s?호점`,
 `오픈\\s?(?:예정|대기)\\s?(?:중인\\s?)?(?:매장|점포)?\\s?(?:[만이은도]\\s?)?${CNT}\\s?(?:개|곳)`,
].join('|');
...
// 모집 범위에서만 보는 공식 규칙 표현(판정기 lib/franchise-compliance.ts가 모집 범위의 match에 더한다. 소비자 문장과 겹치는 모양이라 소비자 범위와 A2 옵트인 경로에는 걸지 않는다).
// 창업비용: 비용 라벨 뒤 단위 없는 금액('인테리어 평당 150', '창업비 3,900'), '총 비용', 금액이 먼저 오는 '4천이면 사장님'·'3천 들고 오세요'·'5천 안쪽으로 오픈'.
// 업계 순위: 지표 뒤 1위('만족도 1위', '가맹점 수 증가율 1등'), '#1'. 연도(2025)·개수·면적·비율은 단위 없는 금액이 아니다.
const FR_BARE_AMOUNT='(?<![\\d,.])(?!(?:19|20)\\d{2}(?![\\d,]))(?:\\d{1,3}(?:,\\d{3})+|\\d{2,})(?![\\d%A-Za-z]|[,.]\\d|\\s?(?:개|명|평|㎡|년|개월|달|일|시간|분|호점|위|등|회|퍼센트|배|잔|곳))';
// 매장 수(모집 범위): 지역·기간으로 좁힌 매장 수('경기도에만 매장 38곳', '서울에서만 45곳 운영', '올해 오픈한 매장만 40개')도 매장 수 주장이다. 원장에 지역·기간 값이 없어 근거 없음이다(R2 5차 재검토).
export const FR_STORE_SUBSET=`(?:(?<![가-힣])[가-힣]{1,6}(?:도|시|구|군|권|지역|지방)|서울|부산|대구|인천|광주|대전|울산|세종|제주|수도권|경기|강원|충청|전라|경상)\\s?(?:에만|에서만|지역에만|에|에서)\\s?(?:만\\s?)?(?:운영\\s?(?:중인\\s?)?)?(?:매장|점포|가맹점|지점|직영점)\\s?(?:이|만|은|수)?\\s?${CNT}\\s?(?:개|곳)`
 +`|(?:(?<![가-힣])[가-힣]{1,6}(?:도|시|구|군|권|지역|지방)|서울|부산|대구|인천|광주|대전|울산|세종|제주|수도권|경기|강원|충청|전라|경상)\\s?(?:에만|에서만)\\s?${CNT}\\s?(?:개|곳)\\s?(?:의\\s?)?(?:매장|점포|가맹점|지점|운영|영업|성업)`
 +`|(?:올해|작년|지난해|올\\s?한\\s?해|이번\\s?해|상반기|하반기|\\d{4}\\s?년)\\s?(?:에\\s?|에만\\s?)?(?:새로\\s?|신규\\s?)?(?:오픈|개점|출점|개설)(?:한|된|했던)\\s?(?:매장|점포|가맹점|지점)\\s?(?:만|이|은|수)?\\s?${CNT}\\s?(?:개|곳)`;
export const FR_RECRUIT_EXTRA:Readonly<Record<string,string>>=Object.freeze({
 'kr.fr.startup_cost_claims':[`(?:${SHARED_COST})\\s?(?:[^.\\n]|\\.(?=\\d)){0,14}?(?:\\d[\\d,.]*\\s?(?:만|천|억|원)|${FR_BARE_AMOUNT})`,'총\\s?(?:비용|금액)(?![가-힣])',`인테리어(?:\\s?(?:비용|비|공사비))?\\s?(?:평당|(?:3\\.3\\s?)?(?:㎡|m2)\\s?당)\\s?(?:약\\s?)?(?:\\d[\\d,.]*\\s?(?:만|천|억|원)|${FR_BARE_AMOUNT})`,
  '(?<![\\d,.])\\d[\\d,.]*\\s?(?:만|억|천\\s?만|천)\\s?원?\\s?(?:(?:안쪽|이하|미만|이내|선|대)\\s?)?(?:(?:으로|이면|로)\\s?(?:매장\\s?|가게\\s?)?(?:창업|오픈|개설|개점|사장|점주|오너|시작)|만\\s?(?:있으면|들고)|들고\\s?(?:오|와))',
  // '소자본 3천대 창업', '실투자금 3,500이면 오픈 가능'(R2 5차 재검토).
  '(?<![\\d,.])\\d[\\d,.]*\\s?(?:만|억|천\\s?만|천)\\s?원?\\s?(?:안쪽|이하|미만|이내|선|대)\\s?(?:창업|오픈|개설|개점|가맹)',
  `(?:실\\s?)?투자\\s?금\\s?(?:은|는|이)?\\s?(?:\\d[\\d,.]*\\s?(?:만|천|억)|${FR_BARE_AMOUNT})\\s?원?\\s?(?:이면|으로|로|만\\s?있으면)`].join('|'),
 'kr.fr.store_count_claims':FR_STORE_SUBSET,
 'kr.fr.superlative_claims':'(?:만족도|선호도|인지도|추천율|재구매율|증가율|성장률|성장세|점유율|매출|판매량|가맹점\\s?수|매장\\s?수|출점\\s?수)\\s?(?:[가-힣]{1,4}\\s)?(?:1\\s?위|1\\s?등|No\\.?\\s?1(?!\\d)|넘버\\s?원)|(?<![\\w#])#\\s?1(?![\\d,.])',
});
export const FR_BARE_AMOUNT_RE=FR_BARE_AMOUNT;
...
// 영어 표현은 대소문자를 가리지 않는다('Profit ✔ Guaranteed ✔', '수익 guarantee 모델', 'Guaranteed 월 500').
const ci=(w:string)=>w.replace(/[a-z]/gi,c=>`[${c.toUpperCase()}${c.toLowerCase()}]`);
const EN_GUAR=`(?<![A-Za-z])${ci('guarantee')}(?:[DdSs])?(?![a-z])`;
const EN_REV=`(?<![A-Za-z])(?:${ci('profit')}s?|${ci('income')}|${ci('revenue')}s?|${ci('return')}s?|${ci('sales')}|${ci('earning')}s?|ROI)(?![a-z])`;
...
// '약속드릴 수 없습니다'·'약속드리지 않습니다'의 '드'는 부정 서술 쪽에 남긴다(부정 판정이 '드릴 수 없'을 본다).
const REV_VERB=`(?:보장|보증(?!\\s?(?:금|보험))|담보(?!\\s?(?:대출|물|권|설정|제공|로|를))|확약|책임\\s?제|개런티|확정\\s?지급|확정(?=\\s?(?:$|[!.,~?]|드립|해\\s?드|됩니다|입니다|이에요|합니다|이다|된다|적인|적으로|보장|창업|가맹|브랜드|시스템))|책임(?:집|져|지겠|진다|질|지고|지며)|(?<=(?:본사|본부|저희)[^.,\\n]{0,15})책임지는|책임지는(?=\\s?(?:창업|가맹|브랜드|본사|본부|시스템|프랜차이즈|구조|사업|아이템|점포))|약속(?:합|해|하겠|하는|된|함|(?=\\s?드))|지켜\\s?(?:드|줍|주|준다)|${EN_GUAR})`;
const REV_EN=`${EN_GUAR}\\s+(?:${ci('your')}\\s+|${ci('monthly')}\\s+|${ci('minimum')}\\s+|${ci('fixed')}\\s+)?${EN_REV}|${EN_REV}\\s*[,:]?\\s+(?:${ci('is')}\\s+|${ci('are')}\\s+)?(?:100\\s?%\\s+)?${EN_GUAR}|${ci('minimum')}\\s+(?:${ci('income')}|${ci('profit')}|${ci('revenue')})\\s+${ci('guarantee')}`;
// 소득으로서의 '수입'은 보장·보증과 바로 붙을 때만 본다('수입 보장'). '원두 수입 확정 소식'은 수입품이다.
// 월 N 보장은 단위 없는 수('월 500 보장', '월 1,000 보장')와 '월 천 보장'도 보고, 사이에 부사·본사 주어·쉼표가 와도 본다('월 400은 기본으로 보장해드려요', '안정적인 월 500, 본사가 약속드립니다').
// 손해·적자·매출 부족·부진, '못 벌면', '목표 못 채우면', '월 300 미만이면'을 본사가 물어 주거나 부족분·차액을 채워·지급하는 표현, 수익이 안 나면 가맹비를 돌려준다는 표현도 보장이다.
// 짧은 문장으로 끊은 보장('월 순수익 걱정 끝. 본사가 보장합니다.', '수익 걱정 NO! 부족분은 본사가 책임집니다')은 판정기가 이은 문장에서 본다.
const REV_BARE='(?:(?:\\d{1,3}(?:,\\d{3})+|\\d{2,})(?![\\d%]|[,.]\\d)|(?:\\d+|[일이삼사오육칠팔구])?\\s?(?:천|백))';
const BARE_GAP=`\\s?(?:(?:기본으로|기본|안정적으로|${REV_ADV})\\s?)?(?:[,，]\\s?)?(?:${REV_SUBJ}(?:가|에서|이|는|께서)?\\s?)?(?:(?:기본으로|${REV_ADV})\\s?)?`;
const REV_PAY='(?:물어\\s?(?:드|줍|주)|메꿔|메워|채워\\s?(?:드|줍|주)|보전|보상|보충|배상|갚아|지급(?:합|해|하|드|됩)|책임(?:집|져|지겠|진다))';
const SHORT_TRIGGER='(?:(?:목표|기준|예상|약속)(?:에|치에|를|을)?\\s?(?:못\\s?미치|미달하|미달되|못\\s?채우|안\\s?되|모자라)(?:면|시면|는\\s?경우|할\\s?경우|일\\s?경우)|(?:수익|매출|장사|순수익)\\s?(?:이|가)?\\s?(?:안|못)\\s?(?:나오|나|생기|되|올라)(?:면|시면|을\\s?경우|을\\s?때|는\\s?달)|못\\s?(?:벌|버)(?:면|시면|는\\s?달)|(?:목표|매출|수익)[^.\\n]{0,8}?못\\s?채우(?:면|시면)|(?:매출|수익|장사)\\s?(?:이|가)?\\s?부진(?:하면|하시면|할\\s?경우|할\\s?때)|(?:월|매출|수익|순익|순수익)\\s?[^.\\n]{0,6}?\\d[\\d,.]*\\s?(?:만\\s?원?|천|원)?\\s?(?:미만|이하)(?:이면|일\\s?경우|일\\s?때|이시면))';
const SHORTFALL='(?:부족분|차액|모자란\\s?(?:금액|만큼|부분|돈)|(?:그\\s?)?차이\\s?(?:만큼|분|금액)?)';
const REV_LOSS=`(?:손해|손실|적자)[^.\\n]{0,15}?${REV_SUBJ}(?:가|에서|이)?[^.\\n]{0,10}?${REV_PAY}`
 +`|${SHORT_TRIGGER}[^.\\n]{0,20}?(?:${REV_SUBJ}(?:가|에서|이|께서)?[^.\\n]{0,10}?${REV_PAY}|(?:가맹비|가맹금|투자금|창업\\s?비용?|교육비|보증금)[^.\\n]{0,10}?(?:환불|반환|돌려\\s?(?:드|줍|받)))`
 +`|(?:매출|수익|순익|순수익|벌|목표|미만|미달|부진)[^\\n]{0,25}?${SHORTFALL}(?:은|는|을|를|이|가)?\\s?(?:${REV_SUBJ}(?:가|에서|이|께서)?\\s?)?(?:전액\\s?|전부\\s?|다\\s?|모두\\s?)?(?:${REV_PAY}|정산(?:해|합|하|드))`
 // 손실·적자가 나면 본사가 부담·정산('손실 나면 본사가 전액 부담합니다'). 사이 낱말 없이 조건절 바로 뒤 본사 주어만 본다('적자 매장 리뉴얼 비용은 본사 부담'은 아니다, R2 5차 재검토).
 +`|(?<!(?:파손|배송|운송|택배|분실|상품|제품|재고|폐기|식자재|원재료)\\s?)(?:손해|손실|적자)\\s?(?:분|액|금)?\\s?(?:이|가|을|를|은|는)?\\s?(?:(?:나|보|생기|발생하|발생되)(?:시)?면|날\\s?경우|발생\\s?시|나도|봐도)?\\s?[,，]?\\s?${REV_SUBJ}(?:가|에서|이|께서)?\\s?(?:전액\\s?|전부\\s?|다\\s?|모두\\s?|100\\s?%\\s?)?(?:부담|정산|보전|책임|메꿔|메워|채워|물어)`;
...
const REV_GUARANTEE=[`${REV_NOUN}${REV_GAP}${REV_VERB}`,'(?<![가-힣])수입(?:을|이)?\\s?(?:100\\s?%\\s?)?(?:보장|보증(?!\\s?(?:금|보험)))',`(?:최저|최소)\\s?(?:수익|매출|수입|소득)[^.,\\n]{0,20}?(?:보장|약속|확정|보증|책임|담보|확약|보전|채워|메워|메꿔|지급)`,REV_LOSS,REV_ELLIPSIS,
 `(?<![\\d가-힣])(?:월|연|매월|매달|한\\s?달)\\s?${REV_BARE}\\s?(?:만\\s?원?)?\\s?(?:[을를이가은는도]|이상|까지)?${BARE_GAP}(?:보장|확정|보증|약속|담보|확약|${EN_GUAR})`,
 `(?:보장|확정|보증|약속)(?:된|되는|하는|해\\s?(?:주는|드리는)|받는)?\\s?(?:순?수익(?:금)?(?!률)|매출(?!\\s?\\d+\\s?위)|수입(?=\\s?(?:[은는이가을를도으]|\\d|$|[,.!]))|소득|순이익|ROI)|${EN_GUAR}\\s?(?:[:：]\\s?)?(?:순?수익|매출|순이익|순익|소득|(?<![\\d가-힣])(?:월|연|매월|매달|한\\s?달)\\s?${REV_BARE})`,
 `(?<![\\d가-힣])(?:월|연|매월|매달|한\\s?달|월평균|연평균|하루)\\s?${REV_AMOUNT}\\s?(?:[을를이가은는도]|이상|까지)?${BARE_GAP}(?:보장|확정|보증|약속|${EN_GUAR})`,
 `(?:적자|영업\\s?손실|매출\\s?(?:미달|부족|감소|하락|부진)|수익\\s?(?:미달|부족|감소|부진)|목표\\s?(?:매출|수익)?\\s?미달)[^.\\n]{0,20}?(?:(?:본사|본부|가맹\\s?본부|저희)[^.\\n]{0,10}?)?${REV_PAY}`,
 `(?:연봉|월급)${REV_GAP}${REV_VERB}(?=[^.\\n]{0,20}(?:창업|가맹|점주|사장|오너))|(?<=(?:창업|가맹|점주|사장|오너)[^.\\n]{0,20})(?:연봉|월급)${REV_GAP}${REV_VERB}`,
 // R2 5차 재검토: 최소 금액을 본사가 지켜 주는 표현('월 수입, 최소 300은 본사가 지켜드립니다'), 본사가 매달 정한 금액을 채워·지급하는 표현('본사가 월 250까지 채워드립니다', '매달 300 드리는 위탁 운영'),
 // 고정·확정 수익과 배당('매달 고정 수익을 드리는', '매달 확정 배당 350', '고정 수익형'). 직원 급여·임대료·광고비·적립·쿠폰·상금·장학금 문장은 아니다.
 `(?:최소|최저|기본)\\s?${REV_BARE}\\s?(?:만\\s?원?)?\\s?(?:은|는|을|를|까지|이상)?\\s?${REV_SUBJ}(?:가|에서|이|께서)?\\s?(?:${REV_ADV}\\s?)?(?:지켜\\s?(?:드|줍|주)|보장|책임|채워|보전|약속|메꿔|메워)`,
 `(?<!(?:인건비|월세|임대료|광고비|홍보비|교육비|급여|월급|연봉|시급|장학금|지원금|상금|적립금|포인트)[^.,\\n]{0,12})(?:${REV_SUBJ}(?:가|에서|이|께서)\\s?(?:${REV_ADV}\\s?)?(?:월|매달|매월|한\\s?달에?|달마다)|(?<![\\d가-힣])(?:매달|매월|달마다))\\s?(?:\\d[\\d,.]*\\s?(?:만|억)\\s?원?|${REV_BARE})\\s?(?:만\\s?원?)?\\s?(?:씩|까지|을|를|이상)?\\s?(?:채워\\s?(?:드|줍|주)|보전|지급(?:해|합|하는|됩)|드리는|드립니다|드려요|드려\\s?(?:요|드)|입금해\\s?드|꽂아\\s?드)(?![^.,\\n]{0,15}(?:적립|쿠폰|포인트|상금|장학|이벤트|경품|할인|기부|급여|월급|연봉|시급|채용|직원|매니저|알바))`,
 '(?:고정|확정)\\s?(?:수익|수입|배당)(?:금)?\\s?(?:을|를)?\\s?(?:드리|드립|드려|지급|주는|줍니다|보장|받으|받는|받아|챙기|챙겨)|(?:고정|확정)\\s?수익\\s?형|확정\\s?배당|배당\\s?(?:금\\s?)?(?:보장|확정)',REV_EN].join('|');
// 지원 표현의 라벨과 '지원' 사이에 금액·꾸밈말·본사 주어가 올 수 있다('인테리어 2천 본사 지원', '장비 풀세트 무상 지원'). R2 4차 재검토.
const SUPPORT_GAP='(?:(?:\\d[\\d,.]*\\s?(?:만|천|억)?\\s?원?|풀\\s?세트|일체|전부|전체|싹\\s?다|모두|(?:본사|본부)(?:가|에서)?)\\s?){0,2}';
// 조건 병기: 실제 요건을 적은 경우만('조건: …', '선착순', '… 한정', '12월 31일까지'). '조건은 상담 시 안내'는 조건을 적지 않은 것이다(R2 5차 재검토). '조건 없이', '아무 조건 없이', '오픈까지 책임집니다'는 조건이 아니다.
const SUPPORT_CONDITION='(?<!무|아무\\s?|가맹\\s?|계약\\s?|창업\\s?)조건(?!\\s?(?:없|無|0|(?:은|는|도)?\\s?(?:상담|문의|별도|추후|개별|전화|협의)))|요건|대상자|대상\\s?[:：]|지원\\s?대상|대상은|대상으로|심사|선착순|\\d+\\s?월\\s?(?:\\d+\\s?일)?\\s?까지|\\d{4}\\s?[-.년]\\s?\\d{1,2}\\s?[-.월]?\\s?(?:\\d{1,2}\\s?일?)?\\s?까지|한정|클래스|수강|원데이';
...
const ASSOC_ORG='(?:(?:가맹점\\s?(?:사업자\\s?)?|가맹\\s?점주\\s?|점주\\s?)(?:단체|협의회|연합(?:회)?|협회|모임|노조|노동\\s?조합|조합)|(?:가맹\\s?)?점주회|(?<![가-힣]\\s?)(?:협의회|사업자\\s?단체|노조|노동\\s?조합)|(?<![가-힣])(?:단체|모임)(?=\\s?(?:활동|가입|결성|참여|탈퇴|만들|만드)))';
// 가입·탈퇴·활동·결성('나가시면', '만드는 순간')과 불이익('재계약 어려워요', '계약 끝', '제외', '지원 혜택 없습니다', '심사에 반영', '공급 순서가 밀립니다', '안 하시는 분만 모십니다'). R2 4차 재검토.
const ASSOC_ACT='(?:가입|미가입|탈퇴|참여|활동|결성|나가|나오|만들|만드|들어가|소속)';
const ASSOC_LOSS='(?:(?:계약|재계약|가맹|거래|공급|지원|혜택|심사|평가|물량|신메뉴|출점|우선\\s?(?:권|순위)|프로모션|배정|발주)[^.\\n]{0,8}?(?:조건|불가|제한|해지|해제|거절|종료|중단|불이익|박탈|취소|제외|항목|요소|대상(?:이|에서)?\\s?(?:아닙|아니|아님)|빠지|빠집|빠져|어려|어렵|힘들|힘듭|곤란|장담\\s?(?:못|어렵|할\\s?수\\s?없)|끝(?:입|이|나|난)|반영(?:됩|돼|되|합|해|하)|밀리|밀립|밀려|늦어|늦춰)|(?:지원|혜택)[^.\\n]{0,4}?(?:없(?:습니다|어요|음|다|어)|끊)|(?:재계약|계약\\s?(?:연장|갱신))\\s?(?:없(?:음|습니다|어요)|안\\s?(?:됨|됩니다|돼요))|(?:안|않)\\s?(?:하시는|하는|하실|하신)\\s?분(?:만|께만)\\s?(?:모십|모집|받|계약|가능|선정)|조건|불이익|페널티|패널티|제재)';
// 조건 표에 적은 미가입('재계약 조건: 점주 단체 미가입', R2 5차 재검토).
const ASSOC_TERMS=`(?:재계약|계약|가맹|심사|선정|출점|지원)\\s?(?:조건|요건|자격|기준)\\s?[:：]?\\s?(?:은|는)?\\s?[^.\\n]{0,8}?${ASSOC_ORG}\\s?(?:미가입|비가입|가입\\s?(?:안\\s?함|안\\s?한|하지\\s?않은|금지|불가)|탈퇴|불참|비회원|활동\\s?(?:금지|없음|안\\s?함|이력\\s?없))`;
const ASSOCIATION_CONDITION=`${ASSOC_TERMS}|${ASSOC_ORG}[^.\\n]{0,20}?(?:(?:가입|탈퇴|참여|활동)(?:을|를)?\\s?(?:해야|하셔야|하여야|할\\s?것)|${ASSOC_ACT}[^.\\n]{0,15}?${ASSOC_LOSS})|(?:노조|노동\\s?조합|협의회|단체)\\s?(?:소속\\s?)?(?:가맹\\s?)?점주(?:님)?(?:은|는|들은)?\\s?${ASSOC_LOSS}`;
// 면제: 가입 여부와 관계없음·차이 없음·같은 조건('계약 조건은 같습니다', '계약에 영향이 없습니다'). 양보로 쓴 '자유지만'은 면제가 아니다.
const ASSOCIATION_EXCEPT='관계\\s?없이|무관하게|무관합니다|상관\\s?없이|자유(?!\\s?(?:지만|이지만|이나|인데|이긴))|같(?:습니다|아요|다|음)|동일|똑같|차이\\s?(?:가\\s?|는\\s?)?없|영향\\s?(?:이|은|을)?\\s?(?:없|주지\\s?않)';
...
export const COMPLIANCE_LEXICON:{version:string;checkedAt:string;platformPolicy:string;sources:Record<string,ComplianceSource>;rules:ComplianceRule[]}={
 version:'compliance-lexicon-2026-09-25.14',
 ...
 rules:[
  ...
  {id:'kr.fr.association_condition',category:'franchise_recruit',severity:'block',title:'가맹점사업자단체 가입·미가입 조건',match:ASSOCIATION_CONDITION,except:ASSOCIATION_EXCEPT,sources:['franchise_act']},
  ...
  {id:'kr.fr.conditional_support',category:'franchise_recruit',severity:'block',title:'조건 없는 지원처럼 보이는 표현',match:`(?:인테리어\\s?(?:비용|비|공사비)?|가맹비|${CLS}교육비|로열티|창업\\s?(?:자금|비용)|장비|집기|간판|오픈\\s?(?:비용|자금|물품)|개설\\s?비용|초기\\s?(?:비용|자금)|식자재|원재료|원\\s?부자재|초도\\s?물품|홍보\\s?(?:비용|비)?|광고\\s?(?:비용|비)|마케팅\\s?(?:비용|비)?|판촉\\s?(?:비용|비|물))\\s?${SUPPORT_GAP}(?:무상\\s?|전액\\s?)?지원|정부\\s?창업\\s?지원|정부\\s?지원\\s?(?:창업|자금|대출)|창업\\s?(?:자금\\s?)?대출\\s?(?:까지\\s?|도\\s?)?(?:연계|알선|지원|연결|소개|주선)|무조건\\s?지원|(?:간판|집기|장비|인테리어)[^.\\n]{0,6}?(?:무상|무료)\\s?(?:로\\s?)?(?:제공|설치|시공|공사|달아\\s?드|해\\s?드|깔아\\s?드|넣어\\s?드|드립|드려)|오픈\\s?지원금|(?:인테리어|오픈|개설|창업|초기|장비|간판|교육|홍보|광고|마케팅|판촉|식자재)\\s?(?:비용|비|자금|물품)?[^.\\n]{0,6}?(?:전액\\s?)?(?:본사|본부)\\s?(?:가\\s?)?(?:전액\\s?|100\\s?%\\s?)?(?:쏩|쏴\\s?드|쏜다|부담|대\\s?(?:줍|줘|드|주|준))`,except:`${SUPPORT_CONDITION}|${SUPPORT_NOT_FRANCHISEE}`,marked:true,sources:['franchise_decree','franchise_false_info_notice']},
  ...
  {id:'kr.fr.trade_area_claims',category:'franchise_recruit',severity:'block',title:'상권 분석·유동인구·경쟁 점포 주장',match:'(?<!(?:골목|동네|지역|전통\\s?시장)\\s?)상권\\s?(?:분석(?!\\s?(?:은|는|을|를)\\s?[^.,\\n]{0,25}?(?:함께|같이|현장|직접|도와|확인합니다|진행합니다|안내합니다))|보장|보호|검증)|유동\\s?인구\\s?(?:하루\\s?|일\\s?)?\\d|경쟁\\s?(?:점포|매장|업체|업소|가게|브랜드|점)\\s?(?:이\\s?|가\\s?)?(?:없|0|제로|無)|경쟁\\s?(?:제로|0|zero|ZERO)(?![\\d%,.])|동종\\s?(?:업종|업체|매장|가게|점포|브랜드)[^.\\n]{0,8}?(?:없|0|제로)',ledgerKey:'^trade_area_source$',marked:true,sources:['franchise_decree','franchise_false_info_notice']},
  {id:'kr.fr.production_claims',category:'franchise_recruit',severity:'block',scope:'recruitment',title:'자체 공장·직접 생산 표현',match:'자체\\s?(?:공장|생산|제조)|(?:자가|자사|저희|우리)\\s?공장|직영\\s?공장|본사\\s?(?:직접\\s?)?(?:공장|생산|제조)|직접\\s?(?:생산|제조|굽|구운|구워|만든|만들)',ledgerKey:'^production_method$',marked:true,sources:['franchise_false_info_notice','franchise_decree']},
