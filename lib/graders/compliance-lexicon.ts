...
export const FR_STORE_COUNT=[
 ...
 `${CNT}\\s?호점\\s?(?:돌파|달성|시대|눈앞|넘|(?:오픈\\s?)?임박)|(?:벌써|어느덧|무려|드디어|전국|누적)\\s?${CNT}\\s?호점|\\d+\\s?(?:년|개월|달)\\s?(?:만에|새|사이|동안)\\s?${CNT}\\s?호점`,
...
export const FR_RECRUIT_EXTRA:Readonly<Record<string,string>>=Object.freeze({
 'kr.fr.startup_cost_claims':[`(?:${SHARED_COST})\\s?(?:[^.\\n]|\\.(?=\\d)){0,14}?(?:\\d[\\d,.]*\\s?(?:만|천|억|원)|${FR_BARE_AMOUNT})`,'총\\s?(?:비용|금액)(?![가-힣])',`인테리어(?:\\s?(?:비용|비|공사비))?\\s?(?:평당|(?:3\\.3\\s?)?(?:㎡|m2)\\s?당)\\s?(?:약\\s?)?(?:\\d[\\d,.]*\\s?(?:만|천|억|원)|${FR_BARE_AMOUNT})`,
  '(?<![\\d,.])\\d[\\d,.]*\\s?(?:만|억|천\\s?만|천)\\s?원?\\s?(?:(?:안쪽|이하|미만|이내|선|대)\\s?)?(?:(?:으로|이면|로)\\s?(?:매장\\s?|가게\\s?)?(?:창업|오픈|개설|개점|사장|점주|오너|시작)|만\\s?(?:있으면|들고)|들고\\s?(?:오|와))'].join('|'),
...
const SHORT_TRIGGER='(?:(?:수익|매출|장사|순수익)\\s?(?:이|가)?\\s?(?:안|못)\\s?(?:나오|나|생기|되|올라)(?:면|시면|을\\s?경우|을\\s?때|는\\s?달)|못\\s?(?:벌|버)(?:면|시면|는\\s?달)|(?:목표|매출|수익)[^.\\n]{0,8}?못\\s?채우(?:면|시면)|(?:매출|수익|장사)\\s?(?:이|가)?\\s?부진(?:하면|하시면|할\\s?경우|할\\s?때)|(?:월|매출|수익|순익|순수익)\\s?[^.\\n]{0,6}?\\d[\\d,.]*\\s?(?:만\\s?원?|천|원)?\\s?(?:미만|이하)(?:이면|일\\s?경우|일\\s?때|이시면))';
const SHORTFALL='(?:부족분|차액|모자란\\s?(?:금액|만큼|부분|돈))';
const REV_LOSS=`(?:손해|손실|적자)[^.\\n]{0,15}?${REV_SUBJ}(?:가|에서|이)?[^.\\n]{0,10}?${REV_PAY}`
 ...
 +`|(?:매출|수익|순익|순수익|벌|목표|미만|미달|부진)[^\\n]{0,25}?${SHORTFALL}(?:은|는|을|를|이|가)?\\s?(?:${REV_SUBJ}(?:가|에서|이|께서)?\\s?)?(?:전액\\s?|전부\\s?|다\\s?|모두\\s?)?${REV_PAY}`;
...
const ASSOC_ORG='(?:(?:가맹점\\s?(?:사업자\\s?)?|가맹\\s?점주\\s?|점주\\s?)(?:단체|협의회|연합회|협회|모임|노조|노동\\s?조합|조합)|(?:가맹\\s?)?점주회|(?<![가-힣]\\s?)(?:협의회|사업자\\s?단체|노조|노동\\s?조합)|(?<![가-힣])(?:단체|모임)(?=\\s?(?:활동|가입|결성|참여|탈퇴|만들|만드)))';
...
const ASSOC_LOSS='(?:(?:계약|재계약|가맹|거래|공급|지원|혜택|심사|평가|물량|신메뉴)[^.\\n]{0,8}?(?:조건|불가|제한|해지|해제|거절|종료|중단|불이익|박탈|취소|제외|빠지|빠집|빠져|어려|힘들|끝(?:입|이|나|난)|반영(?:됩|돼|되|합|해|하)|밀리|밀립|밀려|늦어|늦춰)|(?:지원|혜택)[^.\\n]{0,4}?(?:없(?:습니다|어요|음|다|어)|끊)|(?:재계약|계약\\s?(?:연장|갱신))\\s?(?:없(?:음|습니다|어요)|안\\s?(?:됨|됩니다|돼요))|(?:안|않)\\s?(?:하시는|하는|하실|하신)\\s?분(?:만|께만)\\s?(?:모십|모집|받|계약|가능|선정)|조건|불이익|페널티|패널티|제재)';
...
export const COMPLIANCE_LEXICON:{version:string;checkedAt:string;platformPolicy:string;sources:Record<string,ComplianceSource>;rules:ComplianceRule[]}={
 ...
 rules:[
  ...
  {id:'kr.fr.conditional_support',category:'franchise_recruit',severity:'block',title:'조건 없는 지원처럼 보이는 표현',match:`(?:인테리어\\s?(?:비용|비|공사비)?|가맹비|${CLS}교육비|로열티|창업\\s?(?:자금|비용)|장비|집기|간판|오픈\\s?(?:비용|자금|물품)|개설\\s?비용|초기\\s?(?:비용|자금))\\s?${SUPPORT_GAP}(?:무상\\s?|전액\\s?)?지원|정부\\s?창업\\s?지원|정부\\s?지원\\s?(?:창업|자금|대출)|창업\\s?(?:자금\\s?)?대출\\s?(?:까지\\s?|도\\s?)?(?:연계|알선|지원|연결|소개|주선)|무조건\\s?지원|(?:간판|집기|장비|인테리어)[^.\\n]{0,6}?(?:무상|무료)\\s?(?:로\\s?)?(?:제공|설치|달아\\s?드|해\\s?드|깔아\\s?드|넣어\\s?드|드립|드려)|오픈\\s?지원금|(?:인테리어|오픈|개설|창업|초기|장비|간판|교육)\\s?(?:비용|비|자금|물품)?[^.\\n]{0,6}?(?:전액\\s?)?(?:본사|본부)\\s?(?:가\\s?)?(?:전액\\s?)?(?:부담|대\\s?(?:줍|줘|드|주|준))`,except:`${SUPPORT_CONDITION}|${SUPPORT_NOT_FRANCHISEE}`,marked:true,sources:['franchise_decree','franchise_false_info_notice']},
