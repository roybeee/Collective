...
const PURCHASE_CTA='구매하기|바로\\s?구매|지금\\s?구매|구매하세요|장바구니\\s?(?:에\\s?)?(?:담기|담아|담으|담고|담는|넣기|넣어|추가|버튼)|결제하기|주문하기|스토어에서\\s?(?:구매|주문)|온라인\\s?(?:판매|구매)(?!\\s?(?:와|과|의|실적|재고|매출|비중|목표|데이터|채널|전환|경로|분석|량|액|이며|이다|입니다))';
...
const CTA_HELD=[`${M}[^.\\n]{0,12}(?:보류(?!\\s?없)|미사용)`,`${M}[^.\\n]{0,12}${AFTER_CONFIRM}[^.\\n]{0,12}(?:넣|추가|적|쓰|사용|노출|반영|바꾸|바꾼|바꿉|바꿔|교체|전환|변경)`,
 ...
 `${AFTER_CONFIRM}(?:에만|에)?[^.\\n]{0,20}${M}[^.\\n]{0,4}(?:을|를|으로|로)?\\s?(?:검토|결정)`,`${AFTER_CONFIRM}\\s?검토[^.\\n]{0,6}${M}`].join('|');
