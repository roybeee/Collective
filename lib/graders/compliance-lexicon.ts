...
// '장바구니'는 담기 동작('장바구니 담기·장바구니에 담·장바구니 추가·장바구니 버튼')만 구매 유도다. '장바구니 이후 이탈', '상품 상세·장바구니·결제·주문 확인'처럼 퍼널 단계 명사는 아니다(R3 121791b 재채점 뒤 MAPDAL 실측).
// '구매하기·주문하기·결제하기' 뒤가 '어려움·전·위해'면 행동 유도가 아니라 서술이다('비교·주문하기 어려움'). '판매처'는 판매 채널 명사다(fa9da73 재채점 뒤 ODA·MAPDAL 인사이트).
const NOT_CTA_TAIL='(?!\\s?(?:어렵|어려|힘들|불편|위해|위한|전에|전\\s|직전|까지))';
const PURCHASE_CTA=`구매하기${NOT_CTA_TAIL}|바로\\s?구매|지금\\s?구매|구매하세요|장바구니\\s?(?:에\\s?)?(?:담기|담아|담으|담고|담는|넣기|넣어|추가|버튼)|결제하기${NOT_CTA_TAIL}|주문하기${NOT_CTA_TAIL}|스토어에서\\s?(?:구매|주문)|온라인\\s?(?:판매|구매)(?!\\s?(?:와|과|의|실적|재고|매출|비중|목표|데이터|채널|전환|경로|분석|량|액|처|이며|이다|입니다))`;
...
const CTA_HELD=[`${M}[^.\\n]{0,12}(?:보류(?!\\s?없)|미사용)`,`${M}[^.\\n]{0,12}${AFTER_CONFIRM}[^.\\n]{0,12}(?:넣|추가|적|쓰|사용|노출|반영|바꾸|바꾼|바꿉|바꿔|교체|전환|변경)`,
 `${M}[^.\\n]{0,12}${AFTER_CONFIRM}에?\\s?(?:[)）|]|$)`,
 // 인용 CTA 나열 뒤 '(같은 …) 문구는 … 확인 뒤 추가한다'(2026-09-25 fa9da73 로컬 채널 재실행 S6 전략). 나열 밖 다른 문구('버튼 옆 할인 문구는')의 보류는 아니다.
 `${M}[”’"」』]?(?:\\s?[,，·]\\s?[“‘"「『][^”’"」』\\n]{1,20}[”’"」』])*\\s?(?:(?:같은|등의?)\\s?[^.\\n]{0,12}?)?(?:문구|표현|CTA)(?:은|는|을|를)[^.\\n]{0,24}${AFTER_CONFIRM}[^.\\n]{0,12}(?:넣|추가|적|쓰|사용|노출|반영)`,`${M}[^.\\n]{0,8}(?:사용|노출)\\s?여부[^.\\n]{0,16}(?:결정|판단|검토)`,
 ...
 // 확인·확정 뒤에 검토·결정하는 CTA('확인 후에만 “구매하기”를 검토', '가격 확인 후 ‘구매하기’로 결정', '[가격 확정 후 검토 — 구매하기]'). '확인 후 … 지금 구매하세요'는 아니다.
 `${AFTER_CONFIRM}(?:에만|에)?[^.\\n]{0,20}${M}[^.\\n]{0,4}(?:을|를|으로|로)?\\s?(?:검토|결정|삽입)`,
 // 검수가 남은 CTA를 지적하는 문장('메시지의 ‘구매하기’ 제안이 가격 확인 전 단계에 남아 있어')은 사용이 아니다. '문구가 들어 있는 배너를 게시한다'는 아니다.
 `${M}[^.\\n]{0,2}(?:제안|표현|문구|CTA)(?:이|가)[^.\\n]{0,30}남아\\s?있`,`${AFTER_CONFIRM}\\s?검토[^.\\n]{0,6}${M}`].join('|');
...
export const COMPLIANCE_LEXICON:{version:string;checkedAt:string;platformPolicy:string;sources:Record<string,ComplianceSource>;rules:ComplianceRule[]}={
 version:'compliance-lexicon-2026-09-25.13',
