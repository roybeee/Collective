...
// 부정·배제 표현. 같은 문장에 있으면 금지 표현·광고 표현을 쓴 것으로 보지 않는다.
export const NEGATION=/[가-힣]지\s?(?:않|말)|금지|제외|삭제|무관|아니라|아닌|안\s?됩니다|없습니다|쓰지\s?말/;
...
// 라벨: 제목 줄 또는 문장부호 없는 30자 이하 짧은 줄(예: '고객 가설 3', '게시 카피'). 아래 줄은 가장 가까운 라벨에 속한다.
const isLabel=(l:string)=>heading(l)||(l.trim().length>0&&l.trim().length<=30&&!/[.:!?|‘’“”"]/.test(l)&&!/^[-*>]/.test(l.trim()));
...
export function blocks(text:string):Block[]{
 return text.split('\n').reduce<{label:string;out:Block[]}>((acc,line,index)=>{
  const labelled=isLabel(line),label=labelled?line.replace(/^#+\s*/,'').trim():acc.label;
  return {label,out:[...acc.out,{index,line,label,isLabel:labelled}]};
 },{label:'',out:[]}).out;
}
