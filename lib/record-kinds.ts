...
// 중지 때 재확인 표시는 새 kind 없이 캠페인 이력(event)의 playbookRecheck detail로 남겨 캠페인과 함께 지운다.
 ...
 // 트랙 R R15a-2 모집 자료·행사. 브랜드 행(parent = 브랜드 id)이고 캠페인 id는 참조일 뿐이라 캠페인을 지워도 남는다(승인·내보내기·게시 증빙). 보존 값은 선언이고 구현된 파기는 없다.
 {kind:'recruitment_asset',parent:'brand',campaignDeletion:'not_campaign_scoped',retention:{basis:'policy',anchor:'attributed_lead_evidence',days:null,ref:'귀속 리드 증빙과 같은 기간(휴리스틱) · 구현된 파기 없음'},subjectErasure:'none',description:'가맹 모집 자료 판(행 id <자료 id>:<판>: 유형·원문·원문 SHA-256·근거 사실 id와 판·정보공개서 버전·상태·승인자 id와 역할·체크리스트 버전·내보내기 기록(시각·행위자 id와 역할·해시·판정 버전)·게시 위치 라벨과 확인일·재검토 표시·출처 작업물 id와 판·쓰기 순번). 캠페인과 무관(캠페인 id는 참조). 리드 값·연락처는 담지 않는다'},
 {kind:'recruitment_event',parent:'brand',campaignDeletion:'not_campaign_scoped',retention:{basis:'policy',anchor:'attributed_lead_evidence',days:null,ref:'귀속 리드 증빙과 같은 기간(휴리스틱) · 구현된 파기 없음'},subjectErasure:'none',description:'설명회·견학·박람회(유형·시작 시각·장소 라벨·정원·비용 참조·신청/참석/불참 건수·가명 코드와 상태·연결한 모집 자료 판·상태·판 번호·등록·취소한 사람 id와 역할). 캠페인과 무관(캠페인 id는 참조). 이름·연락처는 담지 않고 가명 코드는 모델 입력에 보내지 않는다(DP-10)'},
