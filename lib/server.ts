...
export function validateCampaign(v:Record<string,unknown>){const fields=['brandId','title','goal','audience','channels','stores','products','startDate','endDate','constraints','sources'] as const;const out={} as Record<string,string|number>;for(const key of fields)out[key]=str(v[key]??'',key==='title'?'캠페인 이름':key==='goal'?'목표':key,key==='sources'?20000:5000,['brandId','title','goal'].includes(key));for(const date of [out.startDate,out.endDate])if(date&&(!/^\d{4}-\d{2}-\d{2}$/.test(String(date))||!Number.isFinite(Date.parse(String(date)))))throw new ApiError(400,'날짜를 확인해 주세요.');if(out.startDate&&out.endDate&&out.startDate>out.endDate)throw new ApiError(400,'종료일은 시작일 이후여야 합니다.');out.budget=num(v.budget??0,'예산');const plan=emptyPlan();for(const k of Object.keys(planFields) as PlanKey[])plan[k]=str((v.plan as Record<string,unknown>)?.[k]??'',planFields[k],5000);(out as unknown as Record<string,unknown>).plan=plan;return out as unknown as Omit<Campaign,'id'|'status'|'version'|'createdAt'|'updatedAt'>}
