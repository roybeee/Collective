...
export async function deleteCampaign(owner:string,input:Record<string,unknown>,by?:EventActor){
 ...
 await db.batch([
  ...
  db.prepare("DELETE FROM records WHERE owner=? AND parent_id=? AND kind IN ('artifact','history','metric','event','viral_experiment','learning_snapshot','team_meeting','hermes_submission','campaign_sequence','openai_submission','role_output_contract','role_output_failure','campaign_directive')").bind(owner,id),
