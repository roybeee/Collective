...
export function researchSteps(id:string,server=false):BrandResearch['steps']{return (server?['identity','customer','channel','investigation'] as const:['investigation'] as const).map(stage=>({id:id+'-'+stage,stage,status:'pending'}))}
export function initialResearch(brand:Brand,id:string,model:string,server=false):BrandResearch{return {id,brandId:brand.id,mode:'deep',execution:server?'server':'interactive',protocol:DEEP_RESEARCH_VERSION,plan:defaultResearchPlan(brand),access:unverifiedResearchAccess(),status:'running',steps:researchSteps(id,server),model,stopRequested:false,createdAt:stamp(),updatedAt:stamp(),tokens:0,snapshot:{brand,observations:[]}}}
