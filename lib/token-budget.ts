// 토큰 예산 가드(loop-4). 소유자가 정한 월 토큰 상한(워크스페이스 1행 + 캠페인별 행)을 HERMES 제출 직전(lib/hermes.ts submitHermes)에 확인한다.
...
// 제출 1건의 예약. 요청 원문은 담지 않는다. keyHash: 멱등 키 sha256 앞 16자(같은 요청의 복구 재전송인지 가른다).
// estimatedTokens(누계에 세는 값) = max(estimatedInputTokens, 같은 종류(kind) 최근 실행의 평균 보고 토큰).
...
async function recentAverage(db:Db,owner:string,kind:UsageKind|null){
 if(!kind)return 0;
 const row=await db.prepare(`SELECT AVG(t) AS n FROM (SELECT ${tokensOf('x')} AS t FROM records x WHERE x.owner=? AND x.kind='provider_usage' AND json_extract(x.data,'$.kind')=? AND ${tokensOf('x')} IS NOT NULL ORDER BY json_extract(x.data,'$.observedAt') DESC LIMIT ${RECENT_RUNS})`).bind(owner,kind).first<{n:number|null}>();
...
export async function reserveTokenBudget(db:Db,owner:string,submissionId:string,saved:{key:string;body:string},now=new Date()){
 const keyHash=await sha16(saved.key),id=reservationKey(owner,submissionId);
 const previous=await db.prepare("SELECT json_extract(data,'$.keyHash') AS keyHash FROM records WHERE id=? AND owner=? AND kind='token_reservation'").bind(id,owner).first<{keyHash:string}>();
 if(previous?.keyHash===keyHash)return {warning:null,created:false};
 const [campaignId,kind]=await Promise.all([submissionCampaign(db,owner,submissionId),submissionKind(db,owner,submissionId)]),estimatedInputTokens=estimateInputTokens(saved.body);
 const reservation:Reservation={submissionId,campaignId,kind,estimatedInputTokens,estimatedTokens:Math.max(estimatedInputTokens,await recentAverage(db,owner,kind)),keyHash,reservedAt:now.toISOString(),runId:null};
 const p=kstMonth(now),{workspace,campaign}=limitsOf(await readBudgets(db,owner),campaignId);
 const limits=[...(workspace!==null?[{limit:workspace,campaignId:null}]:[]),...(campaign!==null?[{limit:campaign,campaignId}]:[])];
 ...
 for(let attempt=0;attempt<2;attempt++){
  ...
  await assertTokenBudget(db,owner,campaignId,reservation.estimatedTokens,{now,excludeSubmissionId:submissionId});
 }
 throw new TokenBudgetExceeded('토큰 예산 초과: 동시에 들어온 다른 제출이 남은 예산을 먼저 예약했습니다. 사용량 화면에서 진행 중 실행을 확인한 뒤 다시 시도하세요.');
}
// 아래 셋은 예약 정리다. 실패해도 이미 끝난 제출·조회를 되돌리지 않는다(남은 예약은 종료 사용량 조인이나 월 경계로 누계에서 빠진다).
